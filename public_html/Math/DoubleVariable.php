...
$thisURL = 'DoubleVariable.php?surveyID=' . $_GET['surveyID'] . '&surveyTitle=' . urlencode($_GET['surveyTitle']);
...
if ($_POST['Action'] == 'VariableMathSubmit') {
	...
	$math = new Math();
	$EnableQCoreClass->setTemplateFile('ShowOptionFile', 'DoubleVariableResult.html');
	$EnableQCoreClass->replace('Covariance', $math->cov($theXValue, $theYValue));
	$Correlation = $math->cor($theXValue, $theYValue);
	$EnableQCoreClass->replace('Correlation', $Correlation);
	$EnableQCoreClass->replace('CorrelationSignificant', $math->corTest($Correlation, count($theXValue)));
	$Regression = $math->lm($theXValue, $theYValue);
	$RegressionHTML = '';

	foreach ($Regression as $var => $value) {
		$RegressionHTML .= $var . ' : ' . $value . '<br/>';
	}

	$EnableQCoreClass->replace('Regression', $RegressionHTML);
	$tTestUnpaired = $math->tTest($theXValue, $theYValue, false);
	$EnableQCoreClass->replace('t-TestUnpaired', $tTestUnpaired);
	$EnableQCoreClass->replace('t-TestUnpairedTest', $math->tDist($tTestUnpaired, (count($theXValue) - 1) * (count($theYValue) - 1)));
	$tTestPaired = $math->tTest($theXValue, $theYValue, true);
	$EnableQCoreClass->replace('t-TestPaired', $tTestPaired);
	$EnableQCoreClass->replace('t-TestPairedTest', $math->tDist($tTestPaired, count($theXValue) - 1));
	$DoubleVariableResultHTML = $EnableQCoreClass->parse('ShowOption', 'ShowOptionFile');
	unset($theXValue);
	unset($theYValue);
	exit($DoubleVariableResultHTML);
}
...
$EnableQCoreClass->setTemplateFile('VariableMathFile', 'DoubleVariable.html');
$questionList = '';
