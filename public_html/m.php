...
if ($_POST['Action'] == 'SurveyOverSubmit') {
	...
	case '1':
		_shownotes($lang['survey_submit'], $S_Row['exitPage'], $lang['survey_gname'] . $S_Row['surveyTitle'], '<ul><li><span class="m">' . $lang['submit_modi_false'] . '</span></li></ul>');
		break;

	case '2':
		$exitMess = nl2br($S_Row['exitTextBody']);
		_shownotes($S_Row['exitTitleHead'], $exitMess, $lang['survey_gname'] . $S_Row['surveyTitle'], '<ul><li><span class="m">' . $lang['submit_modi_false'] . '</span></li></ul>');
		break;

	case '3':
		_shownotes($lang['grade_title'], $lang['submit_modi_false'], $lang['survey_gname'] . $S_Row['surveyTitle']);
		break;

	case '4':
		$theViewResultPage = 'v.php?surveyID=' . $S_Row['surveyID'] . '&qlang=' . $S_Row['lang'];
		_shownotes($lang['survey_submit'], $theViewResultPage, $lang['survey_gname'] . $S_Row['surveyTitle'], '<ul><li><span class="m">' . $lang['submit_modi_false'] . '</span></li></ul>');
		break;
...
if ($thisPageStep != 0) {
	...
	if ($S_Row['status'] != '1') {
		...
		$surveyTplFile = 'uSurveyModi.html';

		if (in_array($S_Row['panelID'], array(30001, 30002, 30003, 30004, 30005, 30006))) {
			$surveyTplFile = 'uSurveyModiSystem.html';
		}
