...
if ($_POST['Action'] == 'DataCrossSubmit') {
	...
	require ROOT_PATH . 'PlugIn/' . $ModuleName . '/Admin/' . $ModuleName . '.cross.inc.php';
	unset($theXArray);
	unset($theXFields);
	unset($theXName);
	unset($theXCond);
...
$thisURL = 'DataCross.php?surveyID=' . $_GET['surveyID'] . '&surveyTitle=' . urlencode($_GET['surveyTitle']);
$EnableQCoreClass->setTemplateFile('DataCrossFile', 'DataCross.html');
$EnableQCoreClass->replace('crossType', 1);
