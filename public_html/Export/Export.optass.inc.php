...
function exportlogic()
{
	...
	$_obf_YfrY8VEd .= ',"问题选项(多值采用英文逗号分割)(问题选项的顺序号)"';
	...
	$_obf_xCnI = ' SELECT DISTINCT a.questionID,a.questionType FROM ' . QUESTION_TABLE . ' a,' . ASSOCIATE_TABLE . ' b WHERE a.questionID = b.questionID AND a.surveyID = \'' . $_GET['surveyID'] . '\' AND b.assType=2 ORDER BY a.orderByID ASC ';
	$_obf_3I8RfSDT = $DB->query($_obf_xCnI);

	while ($_obf_9WwQ = $DB->queryArray($_obf_3I8RfSDT)) {
		$_obf_wDNcGQ__ = ' SELECT a.questionID,a.questionType,a.isHaveOther,a.isNeg,b.* FROM ' . QUESTION_TABLE . ' a,' . ASSOCIATE_TABLE . ' b WHERE a.questionID = b.condOnID AND b.questionID = \'' . $_obf_9WwQ['questionID'] . '\' AND a.surveyID = \'' . $_GET['surveyID'] . '\' AND b.assType=2 ORDER BY b.condOnID ASC ';
		...
		while ($_obf_e1UiKg__ = $DB->queryArray($_obf_xXDux6xl_A__)) {
			...
			$_obf_jisqW3V46L7V141VliIPs54cLnZmauI_ = array();

			switch ($_obf_9WwQ['questionType']) {
			case '2':
			case '24':
				$_obf_0ObUPtbyXdk_ = ' SELECT question_radioID FROM ' . QUESTION_RADIO_TABLE . ' WHERE questionID =\'' . $_obf_9WwQ['questionID'] . '\' ORDER BY optionOptionID ASC ';
				$_obf_ElTJgmvFPYkkqRk_ = $DB->query($_obf_0ObUPtbyXdk_);
				$_obf_juwe = 1;

				while ($_obf_4sSQZ7uAzyQ_ = $DB->queryArray($_obf_ElTJgmvFPYkkqRk_)) {
					$_obf_jisqW3V46L7V141VliIPs54cLnZmauI_[$_obf_4sSQZ7uAzyQ_['question_radioID']] = $_obf_juwe;
					$_obf_juwe++;
				}
			...
			case '3':
			case '25':
				$_obf_Y_4GVwEB5HC_snM_ = ' SELECT question_checkboxID FROM ' . QUESTION_CHECKBOX_TABLE . ' WHERE questionID =\'' . $_obf_9WwQ['questionID'] . '\' ORDER BY optionOptionID ASC ';
				...
				while ($_obf_ipkNkjXCQkeS09k_ = $DB->queryArray($_obf_xkBD7PbRRERPRgWjGvU_)) {
					$_obf_jisqW3V46L7V141VliIPs54cLnZmauI_[$_obf_ipkNkjXCQkeS09k_['question_checkboxID']] = $_obf_juwe;
					$_obf_juwe++;
				}
			...
			case '19':
			case '28':
				$_obf_vM_gnC45po7J = ' SELECT question_range_answerID FROM ' . QUESTION_RANGE_ANSWER_TABLE . ' WHERE questionID =\'' . $_obf_9WwQ['questionID'] . '\' ORDER BY question_range_answerID ASC ';
				$_obf_XYp2i_oZ2zUcdjLB = $DB->query($_obf_vM_gnC45po7J);
				$_obf_juwe = 1;

				while ($_obf_unm31SmvHiwL = $DB->queryArray($_obf_XYp2i_oZ2zUcdjLB)) {
					$_obf_jisqW3V46L7V141VliIPs54cLnZmauI_[$_obf_unm31SmvHiwL['question_range_answerID']] = $_obf_juwe;
					$_obf_juwe++;
				}
			...
			$_obf__WwKzYz1wA__ .= ',"' . $_obf_jisqW3V46L7V141VliIPs54cLnZmauI_[$_obf_e1UiKg__['optionID']] . '"';
			...
			case '7':
				...
				$_obf__WwKzYz1wA__ .= ',"' . $_obf_Z31K1jfRi36v6upgjKFW43JnXegs[$_obf_e1UiKg__['condQtnID']] . '"';
				break;
			...
			case '31':
				$_obf__WwKzYz1wA__ .= ',"' . $_obf_e1UiKg__['condQtnID'] . '"';
				break;
			}
			...
			case '1':
				...
				$_obf__WwKzYz1wA__ .= ',"' . $_obf_3Yd4qKfBZS8EptA6UHeD38CLQpa30ncz[$_obf_e1UiKg__['condOptionID']] . '"';
				break;
			...
			case '25':
				switch ($_obf_e1UiKg__['logicMode']) {
				case '2':
					$_obf__WwKzYz1wA__ .= ',"' . $_obf_e1UiKg__['condOptionID'] . '"';
					break;
			...
			case '30':
				switch ($_obf_e1UiKg__['condOptionID']) {
				case '1':
				default:
					$_obf__WwKzYz1wA__ .= ',"1"';
					break;
...
header('Content-Disposition: attachment; filename=OptAssLogic_' . $_GET['surveyID'] . '_List_' . date('Y-m-d') . '.csv');
echo $logicData;
exit();
