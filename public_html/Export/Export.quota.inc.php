...
function exportlogic()
{
	...
	$_obf_xCnI = ' SELECT DISTINCT quotaID FROM ' . QUOTA_TABLE . ' WHERE surveyID = \'' . $_GET['surveyID'] . '\' ORDER BY quotaID ASC ';
	$_obf_3I8RfSDT = $DB->query($_obf_xCnI);

	while ($_obf_9WwQ = $DB->queryArray($_obf_3I8RfSDT)) {
		$_obf_wDNcGQ__ = ' SELECT a.questionID,a.questionType,a.isHaveOther,a.isNeg,b.* FROM ' . QUESTION_TABLE . ' a,' . CONDITIONS_TABLE . ' b WHERE a.questionID = b.condOnID AND b.quotaID = \'' . $_obf_9WwQ['quotaID'] . '\' AND a.surveyID = \'' . $_GET['surveyID'] . '\' AND b.questionID = 0 ORDER BY b.condOnID ASC ';
		$_obf_xXDux6xl_A__ = $DB->query($_obf_wDNcGQ__);

		while ($_obf_e1UiKg__ = $DB->queryArray($_obf_xXDux6xl_A__)) {
			$_obf__WwKzYz1wA__ .= '"' . $_obf_9WwQ['quotaID'] . '"';
...
header('Content-Disposition: attachment; filename=SurveyQuota_' . $_GET['surveyID'] . '_List_' . date('Y-m-d') . '.csv');
echo $logicData;
exit();
