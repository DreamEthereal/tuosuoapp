...
$SQL = ' SELECT status,surveyID,isCache,surveyName FROM ' . SURVEY_TABLE . ' WHERE surveyID=\'' . $_GET['surveyID'] . '\' ';
...
ob_start();
$ResultList = export_text($_GET['surveyID']);
header('Pragma: no-cache');
header('Last-Modified: ' . gmdate('D, d M Y H:i:s') . ' GMT');
header('Content-Type: application/octet-stream;charset=utf8');
header('Content-Disposition: attachment; filename=Question_' . $Sur_G_Row['surveyName'] . '_List_' . date('Y-m-d') . '.txt');
echo $ResultList;
exit();
