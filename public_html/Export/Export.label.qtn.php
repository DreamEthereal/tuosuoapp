...
if ($_POST['Action'] == 'ExportDataSubmit') {
	...
	case 1:
	default:
		$ResultList = export_label($_POST['surveyID']);
		break;

	case 2:
		if (0 < count($_POST['exportQtnList'])) {
			$ResultList = export_label($_POST['surveyID'], $_POST['exportQtnList']);
		}
	...
	header('Content-Disposition: attachment; filename=SPSS_Label_' . $_POST['surveyID'] . '_Syntax_' . date('Y-m-d') . '.txt');
	echo $ResultList;
	exit();
}
...
$EnableQCoreClass->replace('exportSQL', '');
$questionList = '';
