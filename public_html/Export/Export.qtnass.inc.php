...
function exportlogic()
{
	...
	$_obf_YfrY8VEd .= ',"矩阵行(多值采用英文逗号分割)(矩阵行的顺序号)"';
	...
	$_obf_xCnI = ' SELECT DISTINCT a.questionID,a.questionType FROM ' . QUESTION_TABLE . ' a,' . ASSOCIATE_TABLE . ' b WHERE a.questionID = b.questionID AND a.surveyID = \'' . $_GET['surveyID'] . '\' AND b.assType=1 ORDER BY a.orderByID ASC ';
	$_obf_3I8RfSDT = $DB->query($_obf_xCnI);

	while ($_obf_9WwQ = $DB->queryArray($_obf_3I8RfSDT)) {
		$_obf_wDNcGQ__ = ' SELECT a.questionID,a.questionType,a.isHaveOther,a.isNeg,b.* FROM ' . QUESTION_TABLE . ' a,' . ASSOCIATE_TABLE . ' b WHERE a.questionID = b.condOnID AND b.questionID = \'' . $_obf_9WwQ['questionID'] . '\' AND a.surveyID = \'' . $_GET['surveyID'] . '\' AND b.assType=1 ORDER BY b.condOnID ASC ';
		...
		while ($_obf_e1UiKg__ = $DB->queryArray($_obf_xXDux6xl_A__)) {
			...
			case '26':
			case '27':
				$_obf_UrXeBQ9svomn = ' SELECT question_range_optionID FROM ' . QUESTION_RANGE_OPTION_TABLE . ' WHERE questionID =\'' . $_obf_9WwQ['questionID'] . '\' ORDER BY question_range_optionID ASC ';
				$_obf_wYzGeBYGltIy8kCO = $DB->query($_obf_UrXeBQ9svomn);
				$_obf_juwe = 1;

				while ($_obf_d2_Ii7CmoqkA = $DB->queryArray($_obf_wYzGeBYGltIy8kCO)) {
					$_obf_jisqW3V46L7V141VliIPs54cLnZmauI_[$_obf_d2_Ii7CmoqkA['question_range_optionID']] = $_obf_juwe;
					$_obf_juwe++;
				}
			...
			case '15':
			case '16':
				$_obf_UrXeBQ9svomn = ' SELECT question_rankID FROM ' . QUESTION_RANK_TABLE . ' WHERE questionID =\'' . $_obf_9WwQ['questionID'] . '\' ORDER BY question_rankID ASC ';
				$_obf_wYzGeBYGltIy8kCO = $DB->query($_obf_UrXeBQ9svomn);
				$_obf_juwe = 1;

				while ($_obf_d2_Ii7CmoqkA = $DB->queryArray($_obf_wYzGeBYGltIy8kCO)) {
					$_obf_jisqW3V46L7V141VliIPs54cLnZmauI_[$_obf_d2_Ii7CmoqkA['question_rankID']] = $_obf_juwe;
					$_obf_juwe++;
				}
			...
			$_obf__WwKzYz1wA__ .= ',"' . $_obf_jisqW3V46L7V141VliIPs54cLnZmauI_[$_obf_e1UiKg__['qtnID']] . '"';
...
header('Content-Disposition: attachment; filename=QtnAssLogic_' . $_GET['surveyID'] . '_List_' . date('Y-m-d') . '.csv');
echo $logicData;
exit();
