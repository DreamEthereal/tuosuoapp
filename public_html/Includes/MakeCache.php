...
if ($isSurveyPre == 1) {
	write_to_file(ROOT_PATH . $Config['cacheDirectory'] . '/' . $theSID . '/' . md5('Cache' . $theSID) . '.php', $cacheContent);
}
else {
	write_to_file(ROOT_PATH . $Config['cacheDirectory'] . '/' . $theSID . '/' . md5('Qtn' . $theSID) . '.php', $cacheContent);
	$SQL = ' UPDATE ' . SURVEY_TABLE . ' SET isCache =0 WHERE surveyID=\'' . $theSID . '\' ';
	$DB->query($SQL);
}
