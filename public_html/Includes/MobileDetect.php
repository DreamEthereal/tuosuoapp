...
class Mobile_Detect
{
     ...
     * @var array
     */
	static protected $mobileHeaders = array(
		'HTTP_ACCEPT'                  => array(
			'matches' => array('application/x-obml2d', 'application/vnd.rim.html', 'text/vnd.wap.wml', 'application/vnd.wap.xhtml+xml')
			),
     ...
     * @var array
     */
	static protected $uaHttpHeaders = array('HTTP_USER_AGENT', 'HTTP_X_OPERAMINI_PHONE_UA', 'HTTP_X_DEVICE_USER_AGENT', 'HTTP_X_ORIGINAL_USER_AGENT', 'HTTP_X_SKYFIRE_PHONE', 'HTTP_X_BOLT_PHONE_UA', 'HTTP_DEVICE_STOCK_UA', 'HTTP_X_UCBROWSER_DEVICE_UA');
	/**
	...
	public function setHttpHeaders($httpHeaders = NULL)
	{
		...
		foreach ($httpHeaders as $_obf_Vwty => $_obf_VgKtFeg_) {
			if (substr($_obf_Vwty, 0, 5) == 'HTTP_') {
				$this->httpHeaders[$_obf_Vwty] = $_obf_VgKtFeg_;
			}
		}
	}
	...
	public function getHttpHeader($header)
	{
		...
		$_obf_bLYfpyrgvg_8 = 'HTTP_' . $header;

		if (isset($this->httpHeaders[$header])) {
			return $this->httpHeaders[$header];
		}
		else if (isset($this->httpHeaders[$_obf_bLYfpyrgvg_8])) {
			return $this->httpHeaders[$_obf_bLYfpyrgvg_8];
		}
	}
	...
	{
		if ($type === NULL) {
			$type = self::DETECTION_TYPE_EXTENDED;
		}
	...
	{
		foreach ($this->getMobileHeaders() as $_obf_MpjDP86Kp6UgqFBE => $_obf_RqdAOgToyFQB) {
			if (isset($this->httpHeaders[$_obf_MpjDP86Kp6UgqFBE])) {
				if (is_array($_obf_RqdAOgToyFQB['matches'])) {
					foreach ($_obf_RqdAOgToyFQB['matches'] as $_obf_o9Du_HpB) {
						if (strpos($this->httpHeaders[$_obf_MpjDP86Kp6UgqFBE], $_obf_o9Du_HpB) !== false) {
							return true;
						}
					}
	...
	public function __call($name, $arguments)
	{
		...
		$_obf_Vwty = substr($name, 2);
		return $this->matchUAAgainstKey($_obf_Vwty);
	}
	...
	{
		$key = strtolower($key);
		$_obf_1F42X2QH = array_change_key_case($this->getRules());

		if (array_key_exists($key, $_obf_1F42X2QH)) {
			if (empty($_obf_1F42X2QH[$key])) {
				return NULL;
			}

			return $this->match($_obf_1F42X2QH[$key], $userAgent);
		}
	...
	public function isTablet($userAgent = NULL, $httpHeaders = NULL)
	{
		...
		foreach (self::$tabletDevices as $_obf_Np7bzxK_) {
			if ($this->match($_obf_Np7bzxK_, $userAgent)) {
				return true;
			}
		}
	...
	public function prepareVersionNo($ver)
	{
		...
		$_obf_QWkumkeb = explode('.', $ver, 2);

		if (isset($_obf_QWkumkeb[1])) {
			$_obf_QWkumkeb[1] = @str_replace('.', '', $_obf_QWkumkeb[1]);
		}

		return (double) implode('.', $_obf_QWkumkeb);
	}
	...
	{
		$_obf__WLDeJ5Kid0_ = $this->isMobile();
		if ((4.3 <= $this->version('iPad', self::VERSION_TYPE_FLOAT)) || (3.1 <= $this->version('iPhone', self::VERSION_TYPE_FLOAT)) || (3.1 <= $this->version('iPod', self::VERSION_TYPE_FLOAT)) || ((2.1 < $this->version('Android', self::VERSION_TYPE_FLOAT)) && $this->is('Webkit')) || (7 <= $this->version('Windows Phone OS', self::VERSION_TYPE_FLOAT)) || ($this->is('BlackBerry') && (6 <= $this->version('BlackBerry', self::VERSION_TYPE_FLOAT))) || $this->match('Playbook.*Tablet') || ((1.4 <= $this->version('webOS', self::VERSION_TYPE_FLOAT)) && $this->match('Palm|Pre|Pixi')) || $this->match('hp.*TouchPad') || ($this->is('Firefox') && (12 <= $this->version('Firefox', self::VERSION_TYPE_FLOAT))) || ($this->is('Chrome') && $this->is('AndroidOS') && (4 <= $this->version('Android', self::VERSION_TYPE_FLOAT))) || ($this->is('Skyfire') && (4.1 <= $this->version('Skyfire', self::VERSION_TYPE_FLOAT)) && $this->is('AndroidOS') && (2.3 <= $this->version('Android', self::VERSION_TYPE_FLOAT))) || ($this->is('Opera') && (11 < $this->version('Opera Mobi', self::VERSION_TYPE_FLOAT)) && $this->is('AndroidOS')) || $this->is('MeeGoOS') || $this->is('Tizen') || ($this->is('Dolfin') && (2 <= $this->version('Bada', self::VERSION_TYPE_FLOAT))) || (($this->is('UC Browser') || $this->is('Dolfin')) && (2.3 <= $this->version('Android', self::VERSION_TYPE_FLOAT))) || $this->match('Kindle Fire') || ($this->is('Kindle') && (3 <= $this->version('Kindle', self::VERSION_TYPE_FLOAT))) || ($this->is('AndroidOS') && $this->is('NookTablet')) || ((11 <= $this->version('Chrome', self::VERSION_TYPE_FLOAT)) && !$_obf__WLDeJ5Kid0_) || ((5 <= $this->version('Safari', self::VERSION_TYPE_FLOAT)) && !$_obf__WLDeJ5Kid0_) || ((4 <= $this->version('Firefox', self::VERSION_TYPE_FLOAT)) && !$_obf__WLDeJ5Kid0_) || ((7 <= $this->version('MSIE', self::VERSION_TYPE_FLOAT)) && !$_obf__WLDeJ5Kid0_) || ((10 <= $this->version('Opera', self::VERSION_TYPE_FLOAT)) && !$_obf__WLDeJ5Kid0_)) {
			return self::MOBILE_GRADE_A;
		}

		if (($this->version('iPad', self::VERSION_TYPE_FLOAT) < 4.3) || ($this->version('iPhone', self::VERSION_TYPE_FLOAT) < 3.1) || ($this->version('iPod', self::VERSION_TYPE_FLOAT) < 3.1) || ($this->is('Blackberry') && (5 <= $this->version('BlackBerry', self::VERSION_TYPE_FLOAT)) && ($this->version('BlackBerry', self::VERSION_TYPE_FLOAT) < 6)) || ((5 <= $this->version('Opera Mini', self::VERSION_TYPE_FLOAT)) && ($this->version('Opera Mini', self::VERSION_TYPE_FLOAT) <= 6.5) && ((2.3 <= $this->version('Android', self::VERSION_TYPE_FLOAT)) || $this->is('iOS'))) || $this->match('NokiaN8|NokiaC7|N97.*Series60|Symbian/3') || ((11 <= $this->version('Opera Mobi', self::VERSION_TYPE_FLOAT)) && $this->is('SymbianOS'))) {
			return self::MOBILE_GRADE_B;
		}

		if (($this->version('BlackBerry', self::VERSION_TYPE_FLOAT) < 5) || $this->match('MSIEMobile|Windows CE.*Mobile') || ($this->version('Windows Mobile', self::VERSION_TYPE_FLOAT) <= 5.2)) {
