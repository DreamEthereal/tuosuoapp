...
foreach ($OptionListArray[$questionID] as $question_range_optionID => $theQuestionArray) {
	$combArray[] = $question_range_optionID;
	$skipSQL .= ' b.option_' . $questionID . '_' . $question_range_optionID . ' !=0 AND ';
}
...
foreach ($theCombArray as $thisComb) {
	...
	foreach ($theComb as $question_range_optionID) {
		$comboName .= qnospecialchar($OptionListArray[$questionID][$question_range_optionID]['optionName']) . '<br/>&nbsp;';
		$comboSQL .= ' b.option_' . $questionID . '_' . $question_range_optionID . ' IN (' . $_POST['trueOptions'] . ') OR ';
	}
