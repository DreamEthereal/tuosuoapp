	...
	if ($the_question_range_answerID != '0') {
		if ($isHaveViewCoeff == 1) {
			if ($AnswerListArray[$questionID][$the_question_range_answerID]['isNA'] == 1) {
				$EnableQCoreClass->replace('optionAnswer', qnospecialchar($AnswerListArray[$questionID][$the_question_range_answerID]['optionAnswer']) . ' (不适用)');
			}
			else {
				$EnableQCoreClass->replace('optionAnswer', qnospecialchar($AnswerListArray[$questionID][$the_question_range_answerID]['optionAnswer']) . ' (' . $AnswerListArray[$questionID][$the_question_range_answerID]['optionCoeff'] . '分)');
			}
		}
		else {
