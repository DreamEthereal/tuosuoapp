...
$SQL = ' SELECT * FROM ' . QUESTION_RANGE_LABEL_TABLE . ' WHERE questionID=\'' . $QutRow['questionID'] . '\' ORDER BY question_range_labelID ASC ';
$LabResult = $DB->query($SQL);

while ($LabRow = $DB->queryArray($LabResult)) {
	...
	$DB->query($SQL);
	$multiTextLabelArray[$LabRow['question_range_labelID']] = $DB->_GetInsertID();
}
