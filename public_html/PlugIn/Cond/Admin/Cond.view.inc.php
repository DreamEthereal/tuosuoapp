...
if ($theQtnArray['isSelect'] == 0) {
	if ($R_Row['option_' . $questionID] == '') {
		...
		if ($YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['isNA'] == 1) {
			$EnableQCoreClass->replace('optionName', qnospecialchar($YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['optionName']) . ' (不适用)');
		}
		else {
			$EnableQCoreClass->replace('optionName', qnospecialchar($YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['optionName']) . ' (' . $YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['optionCoeff'] . '分)');
		}
	}
	else {
		$EnableQCoreClass->replace('optionName', qnospecialchar($YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['optionName']));
	}
}
else {
	...
	foreach ($YesNoListArray[$questionID] as $question_yesnoID => $theQuestionArray) {
		...
		if (in_array($question_yesnoID, $option_value_array)) {
			if ($isHaveViewCoeff == 1) {
				$EnableQCoreClass->replace('optionAnswer', '1' . ' (' . $YesNoListArray[$questionID][$question_yesnoID]['optionCoeff'] . '分)');
			}
			else {
