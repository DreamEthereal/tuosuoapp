...
foreach ($RankListArray[$questionID] as $question_rankID => $theQuestionArray) {
	if ($theQtnArray['isCheckType'] != '1') {
		...
		$URL = 'ShowUserDefine.php?type=rating_text&surveyID=' . $surveyID . '&questionID=' . $questionID . '&surveyTitle=' . urlencode($_GET['surveyTitle']) . '&isComb=0&pageNum=' . $_GET['pageID'] . '&dataSourceId=' . $dataSourceId . '&optionName=' . str_replace('+', '%2B', base64_encode(qnospecialchar($theQuestionArray['optionName'])));
	...
	$EnableQCoreClass->replace('type', 1);
	...
	if ($theQtnArray['isHaveOther'] == '1') {
		...
		$EnableQCoreClass->replace('optionID', $question_rankID);
