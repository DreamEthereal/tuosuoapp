...
if ($R_Row['option_' . $questionID] != '0') {
	if ($isHaveViewCoeff == 1) {
		if ($YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['isNA'] == 1) {
			$EnableQCoreClass->replace('optionName', $YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['optionName'] . ' (不适用)');
		}
		else {
			$EnableQCoreClass->replace('optionName', $YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['optionName'] . ' (' . $YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['optionCoeff'] . '分)');
		}
	}
	else {
		$EnableQCoreClass->replace('optionName', $YesNoListArray[$questionID][$R_Row['option_' . $questionID]]['optionName']);
	}
}
