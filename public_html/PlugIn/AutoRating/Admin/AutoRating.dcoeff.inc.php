...
if ($theQtnArray['isSelect'] == 0) {
	$EnableQCoreClass->setTemplateFile('ShowOption' . $theCoeffReportId . 'File', 'RatingCoeffView.html');
	...
	$EnableQCoreClass->setTemplateFile('ShowOption' . $theCoeffReportId . 'File', 'RatingOpenCoeffView.html');
	$EnableQCoreClass->set_CycBlock('ShowOption' . $theCoeffReportId . 'File', 'LIST', 'list' . $questionID);
	$EnableQCoreClass->replace('list' . $questionID, '');
}
...
foreach ($optionArray as $question_checkboxID => $optionName) {
	...
	if ($theQtnArray['isSelect'] == 0) {
		...
		if ($theQtnArray['isSelect'] == 1) {
			$OptionCountSQL = ' SELECT Min(option_' . $questionID . '_' . $question_checkboxID . ') as item_min_answerNum, Max(option_' . $questionID . '_' . $question_checkboxID . ') as item_max_answerNum,Sum(option_' . $questionID . '_' . $question_checkboxID . ') as item_sum_answerNum FROM ' . $table_prefix . 'response_' . $surveyID . ' b WHERE b.option_' . $questionID . '_' . $question_checkboxID . ' != \'0\' AND b.option_' . $questionID . '_' . $question_checkboxID . ' != \'0.00\' and ' . $dataSource;
		}
		else {
		...
		if ($theQtnArray['isSelect'] == 1) {
			...
			$OptionCountSQL = ' SELECT option_' . $questionID . '_' . $question_checkboxID . ',count(*) AS count FROM ' . $table_prefix . 'response_' . $surveyID . ' b WHERE b.option_' . $questionID . '_' . $question_checkboxID . ' != \'0\' and ' . $dataSource;
		}
