...
foreach ($RankListArray[$questionID] as $question_rankID => $theQuestionArray) {
	if ($theQtnArray['isHaveOther'] != '1') {
		...
		$URL = 'ShowUserDefine.php?type=weight_text&surveyID=' . $surveyID . '&questionID=' . $questionID . '&surveyTitle=' . urlencode($_GET['surveyTitle']) . '&isComb=0&pageNum=' . $_GET['pageID'] . '&dataSourceId=' . $dataSourceId . '&optionName=' . str_replace('+', '%2B', base64_encode(qnospecialchar($theQuestionArray['optionName'])));
