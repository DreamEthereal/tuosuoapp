...
foreach ($OptionListArray[$questionID] as $question_range_optionID => $theQuestionArray) {
	if ($theQtnArray['isHaveOther'] != '1') {
		...
		$URL = 'ShowUserDefine.php?type=multiple_text&surveyID=' . $surveyID . '&questionID=' . $questionID . '&surveyTitle=' . urlencode($_GET['surveyTitle']) . '&isComb=1&pageNum=' . $_GET['pageID'] . '&dataSourceId=' . $dataSourceId . '&optionName=' . str_replace('+', '%2B', base64_encode(qnospecialchar($theQuestionArray['optionName'])));
	...
	$OptionCountSQL = ' SELECT COUNT(*) AS optionResponseNum FROM ' . $table_prefix . 'response_' . $_GET['surveyID'] . ' b WHERE b.option_' . $questionID . '_' . $question_range_optionID . ' =\'\' and ' . $dataSource;
		...
		foreach ($CombArray as $combID => $combName) {
			if (!empty($CombOptionIDArray[$combID])) {
				...
				foreach ($CombOptionIDArray[$combID] as $CombOptionID) {
					$Comb_SQL .= ' FIND_IN_SET(' . $CombOptionID . ',b.option_' . $questionID . '_' . $question_range_optionID . ') OR ';
				}
	...
	if (!empty($NoCombArray)) {
		foreach ($NoCombArray as $no_comb_question_range_answerID => $optionAnswer) {
			...
			$OptionCountSQL = ' SELECT COUNT(*) AS optionResponseNum FROM ' . $table_prefix . 'response_' . $_GET['surveyID'] . ' b WHERE FIND_IN_SET( ' . $no_comb_question_range_answerID . ',b.option_' . $questionID . '_' . $question_range_optionID . ') and ' . $dataSource;
			$OptionCountRow = $DB->queryFirstRow($OptionCountSQL);
			$EnableQCoreClass->replace('answerNum', $OptionCountRow['optionResponseNum']);
			$EnableQCoreClass->replace('optionPercent', countpercent($OptionCountRow['optionResponseNum'], $totalResponseNum));
			$EnableQCoreClass->replace('optionValidPercent', countpercent($OptionCountRow['optionResponseNum'], $thisOptionResponseNum));
