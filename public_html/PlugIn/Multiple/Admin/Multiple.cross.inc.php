...
foreach ($OptionListArray[$questionID] as $question_range_optionID => $theQuestionArray) {
	...
	foreach ($theXName as $k => $thisXName) {
		...
		$l = 0;
		$OptionCountSQL = ' SELECT a.question_range_answerID,count(*) as optionResponseNum FROM ' . QUESTION_RANGE_ANSWER_TABLE . ' a,' . $table_prefix . 'response_' . $theSurveyID . ' b WHERE a.questionID = \'' . $questionID . '\' AND FIND_IN_SET(a.question_range_answerID,b.option_' . $questionID . '_' . $question_range_optionID . ') AND ' . $theXCond[$k] . ' and ' . $dataSource;
		...
		$OptionSQL = ' SELECT count(*) as theColTotal FROM ' . $table_prefix . 'response_' . $theSurveyID . ' b WHERE b.option_' . $questionID . '_' . $question_range_optionID . ' != \'\' AND ' . $theXCond[$k] . ' and ' . $dataSource;
	...
	$theOptionID = $question_range_optionID;
