...
$EnableQCoreClass->setTemplateFile('ShowOption' . $questionID . 'File', 'uAutoRange.html');
...
if ($S_Row['isProperty'] == '1') {
	$questionNotes = '[' . $lang['question_type_19'] . ']';
}
...
foreach ($AnswerListArray[$questionID] as $question_range_answerID => $theAnswerArray) {
	...
	$Answer[$j] = $question_range_answerID;
	$j++;
...
foreach ($optionAutoArray as $optionAutoID => $optionAutoName) {
	...
	$remove_value_list .= '	if (document.getElementById(\'range_' . $theBaseID . '_' . $questionID . '_' . $optionAutoID . '\').style.display != \'none\'){' . "\n" . '';
	...
	for (; $i < $AnswerNum; $i++) {
		...
		if ($isModiDataFlag == 1) {
			if (($R_Row['option_' . $questionID . '_' . $optionAutoID] != '') && ($R_Row['option_' . $questionID . '_' . $optionAutoID] == $Answer[$i])) {
			...
			if (($_SESSION['option_' . $questionID . '_' . $optionAutoID] != '') && ($_SESSION['option_' . $questionID . '_' . $optionAutoID] == $Answer[$i])) {
				$EnableQCoreClass->replace('isCheck', 'checked');
			}
			else {
				$EnableQCoreClass->replace('isCheck', '');
	...
	if ($QtnListArray[$questionID]['isRequired'] == '1') {
		$check_survey_form_no_con_list .= '	if (document.getElementById(\'range_' . $theBaseID . '_' . $questionID . '_' . $optionAutoID . '\').style.display != \'none\'){' . "\n" . '';
...
foreach ($AnswerListArray[$questionID] as $question_range_answerID => $theAnswerArray) {
	...
	if ($OptAssCon != '') {
		...
		foreach ($optionAutoArray as $optionAutoID => $optionAutoName) {
			$check_survey_conditions_list .= '		RadioItemUnClick(document.Survey_Form.option_' . $questionID . '_' . $optionAutoID . ',' . $question_range_answerID . ');' . "\n" . '';
		}
...
if (!empty($theBaseQtnArray) && !issamepage($questionID, $theBaseID)) {
	...
	if ($QtnListArray[$questionID]['isNeg'] == 1) {
		$check_survey_conditions_list .= '	getAutoRangeNoSamePageIsNeg(' . $theBaseID . ',\'' . $theOptionAutoIDList . '\',\'' . $theOriValue . '\',' . $questionID . ',' . $startScale . ',' . $endScale . ',' . $theFirstID . ',' . $theLastID . ',' . $isHaveOther . ');' . "\n" . '';
	}
	else {
		$check_survey_conditions_list .= '	getAutoRangeNoSamePage(' . $theBaseID . ',\'' . $theOptionAutoIDList . '\',\'' . $theOriValue . '\',' . $questionID . ',' . $startScale . ',' . $endScale . ',' . $theFirstID . ',' . $theLastID . ',' . $isHaveOther . ');' . "\n" . '';
	}
}
else {
	$theBaseIsSelect = (empty($theBaseQtnArray) ? 0 : $theBaseQtnArray['isSelect']);
	...
	if ($QtnListArray[$questionID]['isNeg'] == 1) {
		$check_survey_conditions_list .= '	getAutoRangeSamePageIsNeg(' . $theBaseID . ',' . $theBaseIsSelect . ',' . $questionID . ',' . $startScale . ',' . $endScale . ',' . $theFirstID . ',' . $theLastID . ',' . $isHaveOther . ');' . "\n" . '';
	}
	else {
		$check_survey_conditions_list .= '	getAutoRangeSamePage(' . $theBaseID . ',' . $theBaseIsSelect . ',' . $questionID . ',' . $startScale . ',' . $endScale . ',' . $theFirstID . ',' . $theLastID . ',' . $isHaveOther . ');' . "\n" . '';
	}
}
