...
if ($_POST['Action'] == 'LogicImportSubmit') {
	...
	$the_ass_qtn_type = array('6', '7', '10', '15', '16', '26', '27');
	...
	while ($csvData = _fgetcsv($File)) {
		if ($csvLineNum != 0) {
			...
			case '26':
			case '27':
				$OptionSQL = ' SELECT question_range_optionID FROM ' . QUESTION_RANGE_OPTION_TABLE . ' WHERE questionID =\'' . $this_question_id . '\' ORDER BY question_range_optionID ASC ';
				$OptionResult = $DB->query($OptionSQL);
				$tmp = 1;

				while ($OptionRow = $DB->queryArray($OptionResult)) {
					$the_ass_option_id_array[$tmp] = $OptionRow['question_range_optionID'];
					$tmp++;
				}
			...
			case '15':
			case '16':
				$OptionSQL = ' SELECT question_rankID FROM ' . QUESTION_RANK_TABLE . ' WHERE questionID =\'' . $this_question_id . '\' ORDER BY question_rankID ASC ';
				$OptionResult = $DB->query($OptionSQL);
				$tmp = 1;

				while ($OptionRow = $DB->queryArray($OptionResult)) {
					$the_ass_option_id_array[$tmp] = $OptionRow['question_rankID'];
					$tmp++;
				}
			...
			case '1':
				...
				foreach ($this_ass_id as $assID) {
					foreach ($this_option_id as $optionID) {
						$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $this_question_id . '\' AND qtnID = \'' . $assID . '\' AND condOnID=\'' . $this_base_id . '\' AND condOptionID=\'' . $optionID . '\' AND assType=1 LIMIT 0,1 ';
						$isHaveRow = $DB->queryFirstRow($SQL);

						if (!$isHaveRow) {
							$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $this_question_id . '\',qtnID = \'' . $assID . '\',condOnID=\'' . $this_base_id . '\',condOptionID=\'' . $optionID . '\',opertion=\'' . $this_opt_id . '\',assType=1,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
							$DB->query($SQL);
							$recNum++;
						}
					}
			...
			case '25':
				...
				case '2':
					...
					foreach ($this_ass_id as $assID) {
						$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $this_question_id . '\' AND qtnID = \'' . $assID . '\' AND condOnID=\'' . $this_base_id . '\' AND condOptionID=\'' . $this_option_id . '\' AND assType=1 LIMIT 0,1 ';
						$isHaveRow = $DB->queryFirstRow($SQL);

						if (!$isHaveRow) {
							$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $this_question_id . '\',qtnID = \'' . $assID . '\',condOnID=\'' . $this_base_id . '\',condOptionID=\'' . $this_option_id . '\',opertion=\'' . $this_opt_id . '\',assType=1,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $this_logic_and . '\',logicMode=2  ';
							$DB->query($SQL);
							$recNum++;
						}
					}
				...
				default:
					...
					foreach ($this_ass_id as $assID) {
						foreach ($this_option_id as $optionID) {
							...
							if (!$isHaveRow) {
								$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $this_question_id . '\',qtnID = \'' . $assID . '\',condOnID=\'' . $this_base_id . '\',condOptionID=\'' . $optionID . '\',opertion=\'' . $this_opt_id . '\',assType=1,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $this_logic_and . '\',logicMode=1 ';
								$DB->query($SQL);
								$recNum++;
							}
						}
			...
			case '7':
				...
				foreach ($this_ass_id as $assID) {
					foreach ($this_qtn_id as $qtnID) {
						foreach ($this_option_id as $optionID) {
							$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $this_question_id . '\' AND qtnID = \'' . $assID . '\' AND condOnID=\'' . $this_base_id . '\' AND condQtnID=\'' . $qtnID . '\' AND condOptionID=\'' . $optionID . '\' AND assType=1 LIMIT 0,1 ';
							$isHaveRow = $DB->queryFirstRow($SQL);

							if (!$isHaveRow) {
								$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $this_question_id . '\',qtnID = \'' . $assID . '\',condOnID=\'' . $this_base_id . '\',condQtnID=\'' . $qtnID . '\',condOptionID=\'' . $optionID . '\',opertion=\'' . $this_opt_id . '\',assType=1,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
								$DB->query($SQL);
								$recNum++;
							}
						}
			...
			case '30':
				...
				foreach ($this_ass_id as $assID) {
					$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $this_question_id . '\' AND qtnID = \'' . $assID . '\' AND condOnID=\'' . $this_base_id . '\' AND assType=1 LIMIT 0,1 ';
					$isHaveRow = $DB->queryFirstRow($SQL);

					if (!$isHaveRow) {
						$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $this_question_id . '\',qtnID = \'' . $assID . '\',condOnID=\'' . $this_base_id . '\',condOptionID=\'' . $this_option_id . '\',opertion=\'' . $this_opt_id . '\',assType=1,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
						$DB->query($SQL);
						$recNum++;
					}
				}
			...
			case '31':
				...
				foreach ($this_ass_id as $assID) {
					foreach ($this_option_id as $optionID) {
						$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $this_question_id . '\' AND qtnID = \'' . $assID . '\' AND condOnID=\'' . $this_base_id . '\' AND condQtnID=\'' . $the_qtn_id . '\' AND condOptionID=\'' . $optionID . '\' AND assType=1 LIMIT 0,1 ';
						$isHaveRow = $DB->queryFirstRow($SQL);

						if (!$isHaveRow) {
							$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $this_question_id . '\',qtnID = \'' . $assID . '\',condOnID=\'' . $this_base_id . '\',condQtnID=\'' . $the_qtn_id . '\',condOptionID=\'' . $optionID . '\',opertion=\'' . $this_opt_id . '\',assType=1,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
							$DB->query($SQL);
							$recNum++;
						}
					}
			...
			case '23':
				...
				foreach ($this_ass_id as $assID) {
					foreach ($this_qtn_id as $qtnID) {
						$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $this_question_id . '\' AND qtnID = \'' . $assID . '\' AND condOnID=\'' . $this_base_id . '\' AND condQtnID=\'' . $qtnID . '\' AND condOptionID=\'' . $this_option_id . '\' AND assType=1 LIMIT 0,1 ';
						$isHaveRow = $DB->queryFirstRow($SQL);

						if (!$isHaveRow) {
							$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $this_question_id . '\',qtnID = \'' . $assID . '\',condOnID=\'' . $this_base_id . '\',condQtnID=\'' . $qtnID . '\',condOptionID=\'' . $this_option_id . '\',opertion=\'' . $this_opt_id . '\',assType=1,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
							$DB->query($SQL);
							$recNum++;
						}
					}
	...
	writetolog('导入矩阵行关联:' . $Sur_G_Row['surveyTitle'] . ':' . $lang['new_import'] . $recNum . $lang['import_num']);
	_showmessage('导入矩阵行关联:' . $Sur_G_Row['surveyTitle'] . ':' . $lang['new_import'] . $recNum . $lang['import_num'], true);
}

if ($_GET['DO'] == 'ImportQtnAss') {
	$EnableQCoreClass->setTemplateFile('LogicImportFile', 'SurveyLogicImport.html');
	$EnableQCoreClass->replace('surveyID', $_GET['surveyID']);
	$EnableQCoreClass->replace('actionType', '矩阵行关联关系');
	$EnableQCoreClass->replace('actionName', 'LogicImportSubmit');
	$EnableQCoreClass->replace('helpFileName', str_replace('+', '%2B', base64_encode('CSV_Sample_QtnAss.csv')));
