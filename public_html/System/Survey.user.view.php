...
if ($_POST['Action'] == 'EditViewUserSubmit') {
	if ($_POST['viewUsers'] != '') {
		...
		for (; $i < count($_POST['viewUsers']); $i++) {
			$SQL = ' INSERT INTO ' . VIEWUSERLIST_TABLE . ' SET administratorsID=\'' . $_POST['viewUsers'][$i] . '\',surveyID=\'' . $_POST['surveyID'] . '\',isAuth=0 ';
			$DB->query($SQL);
		}
	}
...
$EnableQCoreClass->setTemplateFile('SurveyUserEditPageFile', 'SurveyViewUserEdit.html');
...
case '1':
	$UserSQL = ' SELECT administratorsID,administratorsName,nickName,userGroupID,groupType FROM ' . ADMINISTRATORS_TABLE . ' WHERE isAdmin =3 AND groupType=1 ORDER BY administratorsID ASC ';
	break;
...
case '5':
	...
	default:
		if ($AuthRow['userGroupID'] == 0) {
			...
			$UserSQL = ' SELECT administratorsID,administratorsName,nickName,userGroupID,groupType FROM ' . ADMINISTRATORS_TABLE . ' WHERE isAdmin=3 AND groupType=1 AND userGroupID IN (' . implode(',', $theSonGroup) . ') ORDER BY administratorsID ASC ';
		}
		else {
			$theSonSQL = '( concat(\'-\',b.absPath,\'-\') LIKE \'%-' . $AuthRow['userGroupID'] . '-%\' OR b.userGroupID = \'' . $AuthRow['userGroupID'] . '\') ';
			$UserSQL = ' SELECT a.administratorsID,a.administratorsName,a.nickName,a.userGroupID,a.groupType FROM ' . ADMINISTRATORS_TABLE . ' a,' . USERGROUP_TABLE . ' b WHERE a.isAdmin =3 AND ' . $theSonSQL . ' AND a.userGroupID = b.userGroupID AND a.groupType =1 ORDER BY b.absPath ASC,a.administratorsID ASC ';
		}
...
while ($UserRow = $DB->queryArray($UserResult)) {
	$HaveSQL = ' SELECT administratorsID FROM ' . VIEWUSERLIST_TABLE . ' WHERE administratorsID=\'' . $UserRow['administratorsID'] . '\' AND surveyID=\'' . $Row['surveyID'] . '\' AND isAuth=0 LIMIT 0,1 ';
