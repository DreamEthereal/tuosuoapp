...
if ($thisPageStep != 0) {
	...
	if (!isset($_SESSION['adminRoleType']) || ($_SESSION['adminRoleType'] == '4')) {
		$EnableQCoreClass->setTemplateFile($ShowSurveyPageFile, 'InputDataModi.html');
		...
		$EnableQCoreClass->setTemplateFile($ShowSurveyPageFile, 'ResultData.html');
	}
...
if ($thisPageStep == 0) {
	...
	if (!isset($_SESSION['adminRoleType']) || ($_SESSION['adminRoleType'] == '4')) {
		$EnableQCoreClass->setTemplateFile('ShowSurveyFile', 'InputDataModi.html');
		$EnableQCoreClass->replace('survey_URLTitle', urlencode($_GET['surveyTitle']));
		...
		if ($R_Row['taskID'] != '0') {
			$paperFlagInfo = '<tr><td height=5></td></tr><tr><td class="surveybegin">&nbsp;<span class=red style=\'font-family: verdana;\'>*</span>任务名称：<input name="name" id="name" type="text" size=70 value="' . $R_Row['administratorsName'] . '" readonly></td></tr>' . "\n" . '<tr><td class="surveybegin">&nbsp;&nbsp;&nbsp;任务描述：<input name="biaoshi" id="biaoshi" type="text" value="' . $R_Row['ipAddress'] . '" size=50 readonly>&nbsp;&nbsp;<span id="surveyInfo"></span></td></tr>' . "\n" . '<tr><td height=5 style="border-bottom:1px dashed #cccccc;"></td></tr>' . "\n" . '';
			...
			$paperFlagInfo = '<tr><td height=5></td></tr><tr><td class="surveybegin" height=35px>&nbsp;<span class=red style=\'font-family: verdana;\'>*</span>' . $lang['paper_flag'] . '<input name="biaoshi" id="biaoshi" maxlength=15 onkeypress="checkIsAlphabetValid();" style="ime-mode:disabled" onPaste="return false;" type="text" value="' . $R_Row['ipAddress'] . '" size=30 onKeyUp="HttpRequest(\'AjaxCheckPaperFlag.php?surveyID=' . $S_Row['surveyID'] . '&old_biaoshi=' . $R_Row['ipAddress'] . '&biaoshi=\'+this.value);">&nbsp;&nbsp;<span id="surveyInfo"></span></td></tr>' . "\n" . '<tr><td class="surveybegin">&nbsp;&nbsp;&nbsp;' . $lang['paper_panel_flag'] . '<input name="name" id="name" type="text" size=30 value="' . $R_Row['administratorsName'] . '"></td></tr>' . "\n" . '<tr><td height=5 style="border-bottom:1px dashed #cccccc;"></td></tr>' . "\n" . '';
			$EnableQCoreClass->replace('paperFlagInfo', $paperFlagInfo);
			$paperFlagInfoCheck = 'if (!CheckNotNull(document.Survey_Form.biaoshi, \'' . $lang['paper_flag_check'] . '\')){return false;}' . "\n" . '';
			$EnableQCoreClass->replace('paperFlagInfoCheck', $paperFlagInfoCheck);
		}
		...
		$EnableQCoreClass->setTemplateFile('ShowSurveyFile', 'ResultData.html');
		$EnableQCoreClass->replace('paperFlagInfo', '');
		$EnableQCoreClass->replace('paperFlagInfoCheck', '');
	}
