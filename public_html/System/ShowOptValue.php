...
$thisProg = 'ShowOptValue.php?surveyID=' . $_GET['surveyID'] . '&surveyTitle=' . urlencode($_GET['surveyTitle']);
...
if ($_POST['Action'] == 'SettingValueSubmit') {
	require_once 'ShowOptValue.inc.php';
	optvaluedatasubmit();
	$theSID = $_GET['surveyID'];
	require ROOT_PATH . 'Includes/MakeCache.php';
	writetolog($lang['setting_option_value'] . ':' . $Sur_G_Row['surveyTitle']);
	_showsucceed($lang['setting_option_value'] . ':' . $Sur_G_Row['surveyTitle'], $thisProg);
}

$EnableQCoreClass->setTemplateFile('SettingConFile', 'SurveyOptValue.html');
...
if ($Sur_G_Row['status'] != '0') {
	...
	$EnableQCoreClass->replace('cur_4', ' class="cur"');
	$EnableQCoreClass->replace('navActionList', $EnableQCoreClass->parse('SurveyActionPage', 'SurveyActionFile'));
	$theSID = $Sur_G_Row['surveyID'];
	require ROOT_PATH . 'Includes/MakeCache.php';
	require ROOT_PATH . $Config['cacheDirectory'] . '/' . $Sur_G_Row['surveyID'] . '/' . md5('Qtn' . $Sur_G_Row['surveyID']) . '.php';
