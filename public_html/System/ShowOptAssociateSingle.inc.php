...
$ConSQL = ' SELECT DISTINCT condOnID FROM ' . ASSOCIATE_TABLE . ' WHERE questionID = \'' . $theQtnID . '\' AND optionID =\'' . $theOptAssID . '\' AND assType=2 ORDER BY condOnID ASC ';
...
foreach ($ConArray as $condOnID) {
	...
	case '30':
	case '17':
		$ConListSQL = ' SELECT condOptionID,condQtnID,opertion,logicValueIsAnd,logicMode FROM ' . ASSOCIATE_TABLE . ' WHERE condOnID=\'' . $condOnID . '\' AND questionID=\'' . $theQtnID . '\' AND optionID =\'' . $theOptAssID . '\' AND assType=2 ORDER BY condQtnID ASC, condOptionID ASC ';
	...
	case '22':
	case '31':
		$RangeSQL = ' SELECT DISTINCT condQtnID FROM ' . ASSOCIATE_TABLE . ' WHERE condOnID=\'' . $condOnID . '\' AND questionID=\'' . $theQtnID . '\' AND optionID =\'' . $theOptAssID . '\' AND assType=2 ORDER BY condQtnID ASC ';
		...
		while ($RangeRow = $DB->queryArray($RangeResult)) {
			$ConListSQL = ' SELECT condQtnID,condOptionID,opertion,logicValueIsAnd FROM ' . ASSOCIATE_TABLE . ' WHERE condOnID=\'' . $condOnID . '\' AND condQtnID = \'' . $RangeRow['condQtnID'] . '\' AND questionID=\'' . $theQtnID . '\' AND optionID =\'' . $theOptAssID . '\' AND assType=2 ORDER BY condQtnID ASC, condOptionID ASC ';
			$ConListResult = $DB->query($ConListSQL);
			$ConListTotal = $DB->_getNumRows($ConListResult);
			$i = 0;
