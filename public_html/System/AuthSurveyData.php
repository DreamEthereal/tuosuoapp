...
if (($_POST['Action'] == 'SurveyOverSubmit') || ($_POST['Action'] == 'SurveyCacheSubmit')) {
	...
	if ($_POST['Action'] == 'SurveyOverSubmit') {
		...
		$DB->query($SQL);
		$SQL = ' INSERT INTO ' . DATA_TASK_TABLE . ' SET surveyID = \'' . $_POST['surveyID'] . '\',responseID=\'' . $new_responseID . '\',administratorsID=\'' . $_SESSION['administratorsID'] . '\',taskTime=\'' . time() . '\',appStat=0 ';
		...
		case 2:
			$SQL .= ' ,authStat =\'2\',reason=\'(允许录入员修改数据后再进入审核流程)' . "\r\n" . '' . $_POST['reason'] . '\' ';
			break;
...
if ($thisPageStep != 0) {
	...
	$EnableQCoreClass->setTemplateFile($ShowSurveyPageFile, 'ResultAuthData.html');
	...
	if ($thisPageStep != count($pageBreak) - 1) {
		...
		$dataAuthList = '<table width="100%" class="pertable"><tr><td height=25 class="question" valign=center>&nbsp;<span class=red>*</span>请给出您对本回复数据的审核意见：&nbsp;</td></tr><tr><td class="tips">需要对本回复数据给出审核意见</td></tr><tr><td><table cellSpacing=0 cellPadding=0 width="100%"><tr><td class="answer"><input onclick="changeAuthStat()" type="radio" value="1" name="authStat" id="authStat">审核通过<br/><input onclick="changeAuthStat()" type="radio" value="2" name="authStat" id="authStat">审核不通过 (允许录入员修改数据后再进入审核流程)<br/><input onclick="changeAuthStat()" type="radio" value="4" name="authStat" id="authStat">审核不通过 (废卷处理)<br/><input onclick="changeAuthStat()" type="radio" value="3" name="authStat" id="authStat">无法决定，提交给其他人再审核</td></tr></table></td></tr><tr><td height=5 class="surveyclear">&nbsp;</td></tr></table>';
		$data_auth_js_list = '	if (!CheckRadioNoClick(document.Survey_Form.authStat,\'您对本回复数据的审核意见\')){return false;} ' . "\n" . '';
		$theFatherGroup = _getupgroupnode($_SESSION['adminRoleGroupID']);
		$theFatherGroup[] = $_SESSION['adminRoleGroupID'];
		$optUserList = array();
		$OptSQL = ' SELECT a.administratorsID,a.administratorsName,a.nickName,a.userGroupID,a.groupType FROM ' . ADMINISTRATORS_TABLE . ' a, ' . VIEWUSERLIST_TABLE . ' b WHERE a.administratorsID = b.administratorsID AND b.surveyID =\'' . $S_Row['surveyID'] . '\' AND a.isAdmin = 7 AND b.isAuth=1 AND a.userGroupID IN (' . implode(',', $theFatherGroup) . ') AND a.administratorsID != \'' . $_SESSION['administratorsID'] . '\' ORDER BY a.administratorsID ASC ';
		$OptResult = $DB->query($OptSQL);

		while ($OptRow = $DB->queryArray($OptResult)) {
			$optUserList[$OptRow['administratorsID']] = _getuserallname($OptRow['nickName'] . '(' . $OptRow['administratorsName'] . ')', $OptRow['userGroupID'], $OptRow['groupType']);
		}

		$dataAuthList .= '<table width="100%" class="pertable" id="nextUserId_cont" style="display:none"><tr><td height=25 class="question" valign=center>&nbsp;<span class=red>*</span>请给出您准备提交给谁再审核：&nbsp;</td></tr><tr><td class="tips">需要给出\'无法决定，提交给其他人再审核\'审核意见的对应审核员，若没有其他审核员，则您需要作出最终审核结论</td></tr><tr><td><table cellSpacing=0 cellPadding=0 width="100%"><tr><td class="answer"><select name="nextUserId" id="nextUserId"><option value=\'\'>请选择其他审核员...</option>';

		foreach ($optUserList as $nextUserId => $nextUserName) {
			$dataAuthList .= '<option value=\'' . $nextUserId . '\'>' . $nextUserName . '</option>' . "\n" . '';
		}

		$dataAuthList .= '</select></td></tr></table></td></tr><tr><td height=5 class="surveyclear">&nbsp;</td></tr></table>';
		$data_auth_js_list .= '	if( IsInCheckBox(document.Survey_Form.authStat,3)){if (!CheckListNoSelect(document.Survey_Form.nextUserId, \'您准备提交给谁再审核\')) {return false;}} ' . "\n" . '';
		$dataAuthList .= '<table width="100%" class="pertable"><tr><td height=25 class="question" valign=center>&nbsp;<span class=red>*</span>请给出您对本回复数据的审核批注：&nbsp;</td></tr><tr><td class="tips">需要对本回复数据给出具体的审核批注</td></tr><tr><td><table cellSpacing=0 cellPadding=0 width="100%"><tr><td class="answer"><textarea rows=6 cols=130 name="reason" id="reason"></textarea></td></tr></table></td></tr><tr><td height=5 class="surveyclear">&nbsp;</td></tr></table>';
		$data_auth_js_list .= '	if (!CheckNotNull(document.Survey_Form.reason, \'您对本回复数据的审核批注\')) {return false;} ' . "\n" . '';
...
if ($thisPageStep == 0) {
	$EnableQCoreClass->setTemplateFile('ShowSurveyFile', 'ResultAuthData.html');
	$paperFlagInfo = '';
