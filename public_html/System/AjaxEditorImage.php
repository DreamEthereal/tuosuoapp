...
if ($_POST['picType'] == 1) {
	...
	if ($_FILES['picImage']['name'] != '') {
		$time = time();
		$ImageFile_DIR_Name = $Config['absolutenessPath'] . '/PerUserData/p/' . date('Y-m', $time) . '/' . date('d', $time) . '/';
		...
		if (is_uploaded_file($_FILES['picImage']['tmp_name']) && in_array($extension, $FileTypeExt)) {
			...
			$picFileName = 'http://' . $_SERVER['HTTP_HOST'] . substr($_SERVER['SCRIPT_NAME'], 0, -26) . 'PerUserData/p/' . date('Y-m', $time) . '/' . date('d', $time) . '/' . $newFilename;
	...
	case 'flv':
	case 'mp4':
		$viedoIncludeURL = 'http://' . $_SERVER['HTTP_HOST'] . substr($_SERVER['SCRIPT_NAME'], 0, -26);
		echo '<script>parent.AjaxCallBack(\'' . $theFileURL . '\',4,' . $_POST['wmvwidth'] . ',' . $_POST['wmvheight'] . ',\'' . $viedoIncludeURL . '\');</script>';
		break;
	}
}
