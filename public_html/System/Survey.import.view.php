...
$thisProg = 'ShowSurveyList.php?Action=ImportViewUser&surveyID=' . $_GET['surveyID'];
include_once ROOT_PATH . 'Functions/Functions.csv.inc.php';

if ($_POST['Action'] == 'ImportViewUserSubmit') {
	...
	while ($csvData = _fgetcsv($File)) {
		if ($csvLineNum != 0) {
			...
			$isCancelTag = trim($csvData[1]);

			if ($userName == '') {
				continue;
			}

			$SQL = ' SELECT administratorsID FROM ' . ADMINISTRATORS_TABLE . ' WHERE administratorsName = \'' . $userName . '\' AND isAdmin=3 AND groupType=1 ';
			...
			$hSQL = ' SELECT administratorsID FROM ' . VIEWUSERLIST_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND administratorsID = \'' . $theUserId . '\' ';
			$hRow = $DB->queryFirstRow($hSQL);

			if ($hRow) {
				if ($isCancelTag == 1) {
					$SQL = ' DELETE FROM ' . VIEWUSERLIST_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND administratorsID = \'' . $theUserId . '\' ';
					$DB->query($SQL);
					$recNum++;
				}
				else {
				...
				$SQL = ' INSERT INTO ' . VIEWUSERLIST_TABLE . ' SET administratorsID=\'' . $theUserId . '\',surveyID=\'' . $_POST['surveyID'] . '\' ';
				$DB->query($SQL);
				$recNum++;
			}
		}
...
if ($_GET['Action'] == 'ImportViewUser') {
	...
	$EnableQCoreClass->replace('userType', '数据查看');
	$EnableQCoreClass->replace('actionName', 'ImportViewUserSubmit');
	$EnableQCoreClass->replace('csvFileName', str_replace('+', '%2B', base64_encode('CSV_Sample_Viewer.csv')));
