...
$ConSQL = ' SELECT DISTINCT condOnID FROM ' . CONDITIONS_TABLE . ' WHERE questionID = \'' . $theQtnID . '\' ORDER BY condOnID ASC ';
...
foreach ($ConArray as $condOnID) {
	...
	case '30':
	case '17':
		$ConListSQL = ' SELECT optionID,qtnID,opertion,logicValueIsAnd,logicMode FROM ' . CONDITIONS_TABLE . ' WHERE condOnID=\'' . $condOnID . '\' AND questionID=\'' . $theQtnID . '\' ORDER BY qtnID ASC, optionID ASC ';
	...
	case '22':
	case '31':
		$RangeSQL = ' SELECT DISTINCT qtnID FROM ' . CONDITIONS_TABLE . ' WHERE condOnID=\'' . $condOnID . '\' AND questionID=\'' . $theQtnID . '\' ORDER BY qtnID ASC ';
		...
		while ($RangeRow = $DB->queryArray($RangeResult)) {
			$ConListSQL = ' SELECT qtnID,optionID,opertion,logicValueIsAnd FROM ' . CONDITIONS_TABLE . ' WHERE condOnID=\'' . $condOnID . '\' AND qtnID = \'' . $RangeRow['qtnID'] . '\' AND questionID=\'' . $theQtnID . '\' ORDER BY qtnID ASC, optionID ASC ';
			$ConListResult = $DB->query($ConListSQL);
			$ConListTotal = $DB->_getNumRows($ConListResult);
			$i = 0;
