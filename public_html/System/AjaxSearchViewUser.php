...
case '1':
	$UserSQL = ' SELECT administratorsID,administratorsName,nickName,userGroupID,groupType FROM ' . ADMINISTRATORS_TABLE . ' WHERE isAdmin =3 AND groupType=1 AND administratorsName LIKE \'%' . trim($_GET['q_userName']) . '%\' ORDER BY administratorsID ASC ';
	break;
...
case '5':
	...
	default:
		if ($AuthRow['userGroupID'] == 0) {
			...
			$UserSQL = ' SELECT administratorsID,administratorsName,nickName,userGroupID,groupType FROM ' . ADMINISTRATORS_TABLE . ' WHERE isAdmin=3 AND groupType=1 AND userGroupID IN (' . implode(',', $theSonGroup) . ') AND administratorsName LIKE \'%' . trim($_GET['q_userName']) . '%\' ORDER BY administratorsID ASC ';
		}
		else {
			$theSonSQL = '( concat(\'-\',b.absPath,\'-\') LIKE \'%-' . $AuthRow['userGroupID'] . '-%\' OR b.userGroupID = \'' . $AuthRow['userGroupID'] . '\') ';
			$UserSQL = ' SELECT a.administratorsID,a.administratorsName,a.nickName,a.userGroupID,a.groupType FROM ' . ADMINISTRATORS_TABLE . ' a,' . USERGROUP_TABLE . ' b WHERE a.isAdmin =3 AND ' . $theSonSQL . ' AND a.userGroupID = b.userGroupID AND a.groupType =1 AND a.administratorsName LIKE \'%' . trim($_GET['q_userName']) . '%\' ORDER BY b.absPath ASC,a.administratorsID ASC ';
		}
...
while ($UserRow = $DB->queryArray($UserResult)) {
	$HaveSQL = ' SELECT administratorsID FROM ' . VIEWUSERLIST_TABLE . ' WHERE administratorsID=\'' . $UserRow['administratorsID'] . '\' AND surveyID=\'' . $_GET['surveyID'] . '\' AND isAuth=0 LIMIT 0,1 ';
