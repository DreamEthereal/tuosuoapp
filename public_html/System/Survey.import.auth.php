...
$thisProg = 'ShowSurveyList.php?Action=ImportAuthUser&surveyID=' . $_GET['surveyID'];
include_once ROOT_PATH . 'Functions/Functions.csv.inc.php';

if ($_POST['Action'] == 'ImportAuthUserSubmit') {
	...
	while ($csvData = _fgetcsv($File)) {
		if ($csvLineNum != 0) {
			...
			$hSQL = ' SELECT administratorsID FROM ' . VIEWUSERLIST_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND administratorsID = \'' . $theUserId . '\' AND isAuth=1 ';
			$hRow = $DB->queryFirstRow($hSQL);

			if ($hRow) {
				if ($isCancelTag == 1) {
					$SQL = ' DELETE FROM ' . VIEWUSERLIST_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND administratorsID = \'' . $theUserId . '\' AND isAuth=1 ';
					$DB->query($SQL);
				}
				else {
					continue;
				...
				$SQL = ' INSERT INTO ' . VIEWUSERLIST_TABLE . ' SET administratorsID=\'' . $theUserId . '\',surveyID=\'' . $_POST['surveyID'] . '\',isAuth=1 ';
				$DB->query($SQL);
				$recNum++;
			}
		}
...
if ($_GET['Action'] == 'ImportAuthUser') {
	...
	$EnableQCoreClass->replace('actionName', 'ImportAuthUserSubmit');
	$EnableQCoreClass->replace('csvFileName', str_replace('+', '%2B', base64_encode('CSV_Sample_Auther.csv')));
