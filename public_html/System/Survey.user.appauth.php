...
if ($_POST['Action'] == 'EditAuthUserSubmit') {
	if ($_POST['viewUsers'] != '') {
		if ($_POST['nowViewValue'] != '') {
			$SQL = ' DELETE FROM ' . APPEALUSERLIST_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND  administratorsID IN (' . $_POST['nowViewValue'] . ') ';
			$DB->query($SQL);
		}
		...
		for (; $i < count($_POST['viewUsers']); $i++) {
			$SQL = ' INSERT INTO ' . APPEALUSERLIST_TABLE . ' SET administratorsID=\'' . $_POST['viewUsers'][$i] . '\',surveyID=\'' . $_POST['surveyID'] . '\',isAuth=1 ';
			$DB->query($SQL);
		}
	}
...
$EnableQCoreClass->setTemplateFile('SurveyUserEditPageFile', 'SurveyAppAuthUserEdit.html');
...
while ($UserRow = $DB->queryArray($UserResult)) {
	$HaveSQL = ' SELECT administratorsID FROM ' . APPEALUSERLIST_TABLE . ' WHERE administratorsID=\'' . $UserRow['administratorsID'] . '\' AND surveyID=\'' . $Row['surveyID'] . '\' AND isAuth=1 LIMIT 0,1 ';
