...
if ($_POST['Action'] == 'QuotaImportSubmit') {
	...
	while ($csvData = _fgetcsv($File)) {
		if ($csvLineNum != 0) {
			...
			$the_quota_id = (int) trim($csvData[0]);
			...
			if (($the_quota_id == '') || ($the_base_id == '') || ($the_option_id == '')) {
				continue;
			}

			$hSQL = ' SELECT quotaID FROM ' . QUOTA_TABLE . ' WHERE quotaID = \'' . $the_quota_id . '\' LIMIT 1 ';
			...
			case '1':
				...
				foreach ($this_option_id as $optionID) {
					$SQL = ' SELECT conditionsID FROM ' . CONDITIONS_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'0\' AND condOnID=\'' . $this_base_id . '\' AND optionID=\'' . $optionID . '\' AND quotaID=\'' . $this_quota_id . '\' LIMIT 0,1 ';
					$isHaveRow = $DB->queryFirstRow($SQL);

					if (!$isHaveRow) {
						$SQL = ' INSERT INTO ' . CONDITIONS_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'0\',quotaID=\'' . $this_quota_id . '\',administratorsID=\'' . $_SESSION['administratorsID'] . '\',condOnID=\'' . $this_base_id . '\',optionID=\'' . $optionID . '\',opertion=\'' . $this_opt_id . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
						$DB->query($SQL);
						$recNum++;
					}
				}
			...
			case '25':
				...
				case '2':
					...
					$this_logic_and = 0;
					$SQL = ' SELECT conditionsID FROM ' . CONDITIONS_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'0\' AND condOnID=\'' . $this_base_id . '\' AND optionID=\'' . $this_option_id . '\' AND quotaID=\'' . $this_quota_id . '\' LIMIT 0,1 ';
					$isHaveRow = $DB->queryFirstRow($SQL);

					if (!$isHaveRow) {
						$SQL = ' INSERT INTO ' . CONDITIONS_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'0\',quotaID=\'' . $this_quota_id . '\',administratorsID=\'' . $_SESSION['administratorsID'] . '\',condOnID=\'' . $this_base_id . '\',optionID=\'' . $this_option_id . '\',opertion=\'' . $this_opt_id . '\',logicValueIsAnd = \'' . $this_logic_and . '\',logicMode=2 ';
						$DB->query($SQL);
						$recNum++;
					}
				...
				default:
					...
					foreach ($this_option_id as $optionID) {
						...
						if (!$isHaveRow) {
							$SQL = ' INSERT INTO ' . CONDITIONS_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'0\',quotaID=\'' . $this_quota_id . '\',administratorsID=\'' . $_SESSION['administratorsID'] . '\',condOnID=\'' . $this_base_id . '\',optionID=\'' . $optionID . '\',opertion=\'' . $this_opt_id . '\',logicValueIsAnd = \'' . $this_logic_and . '\',logicMode=1 ';
							$DB->query($SQL);
							$recNum++;
						}
					}
			...
			case '7':
				...
				foreach ($this_qtn_id as $qtnID) {
					foreach ($this_option_id as $optionID) {
						$SQL = ' SELECT conditionsID FROM ' . CONDITIONS_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'0\' AND condOnID=\'' . $this_base_id . '\' AND qtnID=\'' . $qtnID . '\' AND optionID=\'' . $optionID . '\' AND quotaID=\'' . $this_quota_id . '\' LIMIT 0,1 ';
						$isHaveRow = $DB->queryFirstRow($SQL);

						if (!$isHaveRow) {
							$SQL = ' INSERT INTO ' . CONDITIONS_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'0\',quotaID=\'' . $this_quota_id . '\',administratorsID=\'' . $_SESSION['administratorsID'] . '\',condOnID=\'' . $this_base_id . '\',qtnID=\'' . $qtnID . '\',optionID=\'' . $optionID . '\',opertion=\'' . $this_opt_id . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
							$DB->query($SQL);
							$recNum++;
						}
					}
			...
			case '30':
				...
				$SQL = ' SELECT conditionsID FROM ' . CONDITIONS_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'0\' AND condOnID=\'' . $this_base_id . '\' AND quotaID=\'' . $this_quota_id . '\' LIMIT 0,1 ';
				$isHaveRow = $DB->queryFirstRow($SQL);

				if (!$isHaveRow) {
					$SQL = ' INSERT INTO ' . CONDITIONS_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'0\',     administratorsID=\'' . $_SESSION['administratorsID'] . '\',condOnID=\'' . $this_base_id . '\',optionID=\'' . $this_option_id . '\',quotaID=\'' . $this_quota_id . '\',opertion=\'' . $this_opt_id . '\' ';
					$DB->query($SQL);
				}
			...
			case '31':
				...
				foreach ($this_option_id as $optionID) {
					$SQL = ' SELECT conditionsID FROM ' . CONDITIONS_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'0\' AND condOnID=\'' . $this_base_id . '\' AND qtnID=\'' . $the_qtn_id . '\' AND optionID=\'' . $optionID . '\' AND quotaID=\'' . $this_quota_id . '\' LIMIT 0,1 ';
					$isHaveRow = $DB->queryFirstRow($SQL);

					if (!$isHaveRow) {
						$SQL = ' INSERT INTO ' . CONDITIONS_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'0\',quotaID=\'' . $this_quota_id . '\',administratorsID=\'' . $_SESSION['administratorsID'] . '\',condOnID=\'' . $this_base_id . '\',qtnID=\'' . $the_qtn_id . '\',optionID=\'' . $optionID . '\',opertion=\'' . $this_opt_id . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
						$DB->query($SQL);
						$recNum++;
					}
				}
			...
			case '4':
				...
				$this_option_id = (int) $the_option_id;
				$this_logic_and = 0;
				$SQL = ' SELECT conditionsID FROM ' . CONDITIONS_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'0\' AND condOnID=\'' . $this_base_id . '\' AND optionID=\'' . $the_option_id . '\' AND quotaID=\'' . $this_quota_id . '\' LIMIT 0,1 ';
				$isHaveRow = $DB->queryFirstRow($SQL);

				if (!$isHaveRow) {
					$SQL = ' INSERT INTO ' . CONDITIONS_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'0\',quotaID=\'' . $this_quota_id . '\',administratorsID=\'' . $_SESSION['administratorsID'] . '\',condOnID=\'' . $this_base_id . '\',optionID=\'' . $the_option_id . '\',opertion=\'' . $this_opt_id . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
					$DB->query($SQL);
					$recNum++;
				}
			...
			case '23':
				...
				foreach ($this_qtn_id as $qtnID) {
					$SQL = ' SELECT conditionsID FROM ' . CONDITIONS_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'0\' AND condOnID=\'' . $this_base_id . '\' AND qtnID=\'' . $qtnID . '\' AND optionID=\'' . $this_option_id . '\' AND quotaID=\'' . $this_quota_id . '\' LIMIT 0,1 ';
					$isHaveRow = $DB->queryFirstRow($SQL);

					if (!$isHaveRow) {
						$SQL = ' INSERT INTO ' . CONDITIONS_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'0\',quotaID=\'' . $this_quota_id . '\',administratorsID=\'' . $_SESSION['administratorsID'] . '\',condOnID=\'' . $this_base_id . '\',qtnID=\'' . $qtnID . '\',optionID=\'' . $this_option_id . '\',opertion=\'' . $this_opt_id . '\',logicValueIsAnd = \'' . $this_logic_and . '\' ';
						$DB->query($SQL);
						$recNum++;
					}
				}
	...
	writetolog('导入配额定义:' . $Sur_G_Row['surveyTitle'] . ':' . $lang['new_import'] . $recNum . $lang['import_num']);
	_showmessage('导入配额定义:' . $Sur_G_Row['surveyTitle'] . ':' . $lang['new_import'] . $recNum . $lang['import_num'], true);
}

if ($_GET['DO'] == 'ImportQuota') {
	$EnableQCoreClass->setTemplateFile('LogicImportFile', 'SurveyLogicImport.html');
	$EnableQCoreClass->replace('surveyID', $_GET['surveyID']);
	$EnableQCoreClass->replace('actionType', '配额定义');
	$EnableQCoreClass->replace('actionName', 'QuotaImportSubmit');
	$EnableQCoreClass->replace('helpFileName', str_replace('+', '%2B', base64_encode('CSV_Sample_Quota.csv')));
