...
$thisProg = 'ShowOptAssociate.php?surveyID=' . $_GET['surveyID'] . '&surveyTitle=' . urlencode($_GET['surveyTitle']);
...
if ($_GET['DO'] == 'ImportOptAss') {
	require 'Survey.import.optass.php';
}

if ($_GET['DOes'] == 'DeleLogic') {
	if (isset($_GET['questionID']) && ($_GET['questionID'] != 0) && ($_GET['questionID'] != '')) {
		$SQL = ' DELETE FROM ' . ASSOCIATE_TABLE . ' WHERE questionID = \'' . $_GET['questionID'] . '\' AND assType=2 AND surveyID=\'' . $_GET['surveyID'] . '\' ';
		$DB->query($SQL);
	}
...
if ($_POST['DeleteSurveyLogicSubmit']) {
	if (is_array($_POST['questionID']) && !empty($_POST['questionID'])) {
		...
		if (($questionIDLists != '0') && ($questionIDLists != '')) {
			$SQL = ' DELETE FROM ' . ASSOCIATE_TABLE . ' WHERE questionID IN (' . $questionIDLists . ') AND assType=2 AND surveyID=\'' . $_POST['surveyID'] . '\' ';
			$DB->query($SQL);
...
if (($_POST['Action'] == 'AddLogicSubmit') || ($_POST['Action'] == 'EditLogicSubmit')) {
	if (isset($_POST['logicMode']) && ($_POST['logicMode'] != 0)) {
		...
		case '1':
			foreach ($_POST['base_optionID'] as $optionID) {
				foreach ($_POST['optionID_1'] as $condOptionID) {
					$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $_POST['questionID'] . '\' AND optionID = \'' . $optionID . '\' AND condOnID=\'' . $_POST['condOnID'] . '\' AND condOptionID=\'' . $condOptionID . '\' AND assType=2 LIMIT 0,1 ';
					$isHaveRow = $DB->queryFirstRow($SQL);

					if (!$isHaveRow) {
						$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $_POST['questionID'] . '\',optionID = \'' . $optionID . '\',condOnID=\'' . $_POST['condOnID'] . '\',condOptionID=\'' . $condOptionID . '\',opertion=\'' . $_POST['opertion_1'] . '\',assType=2,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $_POST['logicValueIsAnd'] . '\',logicMode=1 ';
						$DB->query($SQL);
					}
				}
			}
		...
		case '2':
			foreach ($_POST['base_optionID'] as $optionID) {
				$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $_POST['questionID'] . '\' AND optionID = \'' . $optionID . '\' AND condOnID=\'' . $_POST['condOnID'] . '\' AND condOptionID=\'' . $_POST['optionID_2'] . '\' AND assType=2 LIMIT 0,1 ';
				$isHaveRow = $DB->queryFirstRow($SQL);

				if (!$isHaveRow) {
					$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $_POST['questionID'] . '\',optionID = \'' . $optionID . '\',condOnID=\'' . $_POST['condOnID'] . '\',condOptionID=\'' . $_POST['optionID_2'] . '\',opertion=\'' . $_POST['opertion_2'] . '\',assType=2,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'0\',logicMode=2 ';
					$DB->query($SQL);
				}
			}
		...
		if (isset($_POST['questionID']) && is_array($_POST['base_optionID'])) {
			foreach ($_POST['base_optionID'] as $optionID) {
				if (is_array($_POST['optionID']) && !empty($_POST['optionID'])) {
					foreach ($_POST['optionID'] as $condOptionID) {
						if (is_array($_POST['qtnID']) && !empty($_POST['qtnID'])) {
							foreach ($_POST['qtnID'] as $condQtnID) {
								$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $_POST['questionID'] . '\' AND optionID = \'' . $optionID . '\' AND condOnID=\'' . $_POST['condOnID'] . '\' AND condQtnID=\'' . $condQtnID . '\' AND condOptionID=\'' . $condOptionID . '\' AND assType=2 LIMIT 0,1 ';
								$isHaveRow = $DB->queryFirstRow($SQL);

								if (!$isHaveRow) {
									$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $_POST['questionID'] . '\',optionID = \'' . $optionID . '\',condOnID=\'' . $_POST['condOnID'] . '\',condQtnID=\'' . $condQtnID . '\',condOptionID=\'' . $condOptionID . '\',opertion=\'' . $_POST['opertion'] . '\',assType=2,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $_POST['logicValueIsAnd'] . '\' ';
									$DB->query($SQL);
								}
							}
						}
							...
							if (!$isHaveRow) {
								$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $_POST['questionID'] . '\',optionID = \'' . $optionID . '\',condOnID=\'' . $_POST['condOnID'] . '\',condOptionID=\'' . $condOptionID . '\',opertion=\'' . $_POST['opertion'] . '\',assType=2,administratorsID=\'' . $_SESSION['administratorsID'] . '\',logicValueIsAnd = \'' . $_POST['logicValueIsAnd'] . '\' ';
								$DB->query($SQL);
							}
						}
					}
					...
					if (count($theValidValue) != 0) {
						foreach ($theValidValue as $nodeID) {
							$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $_POST['questionID'] . '\' AND optionID = \'' . $optionID . '\' AND condOnID=\'' . $_POST['condOnID'] . '\' AND condQtnID=\'' . $_POST['qtnID'] . '\' AND condOptionID=\'' . $nodeID . '\' AND assType=2 LIMIT 0,1 ';
							$isHaveRow = $DB->queryFirstRow($SQL);

							if (!$isHaveRow) {
								$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $_POST['questionID'] . '\',optionID = \'' . $optionID . '\',condOnID=\'' . $_POST['condOnID'] . '\',condQtnID=\'' . $_POST['qtnID'] . '\',condOptionID=\'' . $nodeID . '\',opertion=\'' . $_POST['opertion'] . '\',assType=2,administratorsID=\'' . $_SESSION['administratorsID'] . '\' ';
								$DB->query($SQL);
							}
						}
					}
					...
					$SQL = ' SELECT associateID FROM ' . ASSOCIATE_TABLE . ' WHERE surveyID=\'' . $_POST['surveyID'] . '\' AND questionID=\'' . $_POST['questionID'] . '\' AND optionID = \'' . $optionID . '\' AND condOnID=\'' . $_POST['condOnID'] . '\' AND assType=2 LIMIT 0,1 ';
					$isHaveRow = $DB->queryFirstRow($SQL);

					if (!$isHaveRow) {
						$SQL = ' INSERT INTO ' . ASSOCIATE_TABLE . ' SET surveyID=\'' . $_POST['surveyID'] . '\',questionID=\'' . $_POST['questionID'] . '\',optionID = \'' . $optionID . '\',condOnID=\'' . $_POST['condOnID'] . '\',condOptionID=\'' . $_POST['optionID'] . '\',opertion=\'' . $_POST['opertion'] . '\',assType=2,administratorsID=\'' . $_SESSION['administratorsID'] . '\' ';
						$DB->query($SQL);
					}
				}
			}
...
if ($_GET['DO'] == 'AddSingleQtnAss') {
	$EnableQCoreClass->setTemplateFile('SurveyLogicNewFile', 'SurveyOptAssociateNew.html');
	...
	case '24':
		...
		while ($OptionRow = $DB->queryArray($OptionResult)) {
			...
			$base_optionID_list .= '<option value=' . $OptionRow['question_radioID'] . '>' . $optionName . '</option>';
		}
	...
	case '25':
		...
		while ($OptionRow = $DB->queryArray($OptionResult)) {
			$optionName = qnohtmltag($OptionRow['optionName'], 1);
			$base_optionID_list .= '<option value=' . $OptionRow['question_checkboxID'] . '>' . $optionName . '</option>';
		}
	...
	case '28':
		...
		while ($OptionRow = $DB->queryArray($OptionResult)) {
			...
			$base_optionID_list .= '<option value=' . $OptionRow['question_range_answerID'] . '>' . $optionName . '</option>';
		}

		break;
	}

	$EnableQCoreClass->replace('base_optionID_list', $base_optionID_list);
...
if ($_GET['DO'] == 'AddQtnAssNew') {
	$EnableQCoreClass->setTemplateFile('SurveyLogicAddFile', 'SurveyOptAssociateAdd.html');
	$SQL = ' SELECT questionID,questionName,questionType FROM ' . QUESTION_TABLE . ' WHERE surveyID=\'' . $_GET['surveyID'] . '\' AND isPublic=\'1\' AND questionType IN (2,3,6,7,19,24,25,28) ORDER BY orderByID ASC ';
	$Result = $DB->query($SQL);
	$questionIDList = '';
...
$EnableQCoreClass->setTemplateFile('SurveyAssFile', 'SurveyOptAssociate.html');
...
$EnableQCoreClass->replace('importLogicURL', $thisProg . '&DO=ImportOptAss');
$theLogicNum = 0;
$Question = array();
