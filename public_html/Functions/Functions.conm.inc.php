...
function isSkipPage($pageTag, $arrow = 1)
{
	...
	if (empty($pageQtnList[$pageTag]) || ($pageQtnList[$pageTag] == '')) {
		...
		foreach ($pageQtnList[$pageTag] as $theQtnID) {
			...
			if (!ischeckboxselect('99999', $thisBaseID)) {
				continue;
			}
...
{
	global $R_Row;
	if (($R_Row['option_' . $questionID] != '') && ($R_Row['option_' . $questionID] == $checkID)) {
		return 1;
	}
	else {
		return 0;
...
function isradiounselect($checkID, $questionID)
{
	...
	if ($R_Row['option_' . $questionID] != '') {
		if ($R_Row['option_' . $questionID] != $checkID) {
			return 1;
		}
		else {
			return 0;
...
function ischeckboxselect($checkID, $questionID)
{
	...
	if ($R_Row['option_' . $questionID] != '') {
		if (is_array($R_Row['option_' . $questionID])) {
			if (in_array($checkID, $R_Row['option_' . $questionID])) {
				return 1;
			}
			else {
				return 0;
			...
			$_obf_HeZlwr2X7nQH7K10bA__ = explode(',', $R_Row['option_' . $questionID]);
...
{
	global $R_Row;
	$_obf_fVE9owv_EIg_ = count(explode(',', $R_Row[$filedName]));
...
function isnumbertextlogictrue($filedName, $opertion, $checkID, $weight = 1, $isSkipUnkown = 0, $nullType = 1)
{
	...
	if ($nullType == 1) {
		if (!isset($R_Row[$filedName]) || ($R_Row[$filedName] == '')) {
			return 0;
		}
	}
	else {
		if (!isset($R_Row[$filedName]) || ($R_Row[$filedName] == '') || ($R_Row[$filedName] == '0')) {
			return 0;
		}
	}

	if (($isSkipUnkown == 1) && ($R_Row[$filedName] == '99')) {
		return 0;
	}

	$_obf_fVE9owv_EIg_ = $R_Row[$filedName] * $weight;
		...
		foreach ($Matches as $Value) {
			...
			if (!in_array($QtnListArray[$theQtnID]['questionType'], array(2, 3, 4, 17, 18, 23, 24, 25))) {
				...
				if (isSamePage($theQtnID, $questionID)) {
					...
					case '2':
						if (count($theQtnArray) == 2) {
							if (!isset($R_Row['option_' . $theQtnID]) || ($R_Row['option_' . $theQtnID] == '')) {
								$theQtnName = str_replace($Value[0], '', $theQtnName);
							}
							else {
								if (($R_Row['option_' . $theQtnID] == '0') && ($R_Row['TextOtherValue_' . $theQtnID] != '')) {
									$theQtnName = str_replace($Value[0], qshowquotechar($QtnListArray[$theQtnID]['otherText']), $theQtnName);
								}
								else {
									$theQtnName = str_replace($Value[0], qshowquotechar($RadioListArray[$theQtnID][$R_Row['option_' . $theQtnID]]['optionName']), $theQtnName);
								}
							}
						}
						else if ($theQtnArray[2] != 0) {
							...
							if (($R_Row['option_' . $theQtnID] == '0') && ($R_Row['TextOtherValue_' . $theQtnID] != '')) {
								$theQtnName = str_replace($Value[0], $R_Row['TextOtherValue_' . $theQtnID], $theQtnName);
							}
							else {
								$theQtnName = str_replace($Value[0], '', $theQtnName);
							}
					...
					case '23':
						if (count($theQtnArray) != 3) {
							...
							$theQtnName = str_replace($Value[0], $R_Row['option_' . $theQtnID . '_' . $theQtnArray[2]], $theQtnName);
						}

						break;

					case '4':
						$theQtnName = str_replace($Value[0], $R_Row['option_' . $theQtnID], $theQtnName);
						break;
					...
					case '24':
						if (count($theQtnArray) == 2) {
							...
							$theQtnName = str_replace($Value[0], $R_Row['TextOtherValue_' . $theQtnID . '_' . $theQtnArray[2]], $theQtnName);
						}
					...
					case '17':
						if ($QtnListArray[$theQtnID]['isSelect'] != 1) {
							...
							else if ($R_Row['option_' . $theQtnID] == '0') {
								$theQtnName = str_replace($Value[0], qshowquotechar($QtnListArray[$theBaseID]['otherText']), $theQtnName);
							}
							else {
								$theQtnName = str_replace($Value[0], qshowquotechar($theCheckBoxListArray[$R_Row['option_' . $theQtnID]]['optionName']), $theQtnName);
							}
						}
					...
					case '18':
						if ($QtnListArray[$theQtnID]['isSelect'] == 1) {
							...
							if (!isset($R_Row['option_' . $theQtnID]) || ($R_Row['option_' . $theQtnID] == '')) {
								...
								$theQtnName = str_replace($Value[0], qshowquotechar($YesNoListArray[$theQtnID][$R_Row['option_' . $theQtnID]]['optionName']), $theQtnName);
							}
						}
			...
			if (!in_array($QtnListArray[$theQtnID]['questionType'], array(23, 27))) {
				...
				case '23':
					...
					foreach ($YesNoListArray[$theQtnID] as $question_yesnoID => $theQuestionArray) {
						...
						if (isset($R_Row[$theOptionID]) && ($R_Row[$theOptionID] != '')) {
							$theExistValueArray[] = $R_Row[$theOptionID];
						}
					}
