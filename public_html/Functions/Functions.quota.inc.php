<?php
//dezend by http://www.yunlu99.com/
function _getsurveyquotacond($questionID, $surveyID, $surveyLang)
{
	global $DB;
	global $Module;
	global $theMgtFunc;
	global $theResponseID;
	global $QuotaListArray;
	global $QtnListArray;
	$theQtnQuotaRelArray = array();

	foreach ($QuotaListArray as $theQtnQuotaID => $theQtnQuotaArray) {
		$theKeyArray = array_keys($theQtnQuotaArray);
		$theLastQuestionID = $theKeyArray[count($theKeyArray) - 1];

		if ($theLastQuestionID == $questionID) {
			$theQtnQuotaRelArray[] = $theQtnQuotaID;
		}
	}

	if (empty($theQtnQuotaRelArray)) {
		return '';
	}

	$endConList = '';

	foreach ($theQtnQuotaRelArray as $theQuotaID) {
		$j = 0;
		$conList = '';

		foreach ($QuotaListArray[$theQuotaID] as $condOnID => $theCondOnIDArray) {
			$condOnID_QuestionType = $QtnListArray[$condOnID]['questionType'];

			switch ($condOnID_QuestionType) {
			case '1':
			case '2':
			case '3':
			case '4':
			case '24':
			case '25':
			case '30':
			case '17':
				$Con_Total = count($theCondOnIDArray[0]);
				$i = 0;

				if (2 <= $Con_Total) {
					$conList .= '(';
				}

				foreach ($theCondOnIDArray[0] as $theOptionArray) {
					$i++;

					if ($condOnID_QuestionType == '4') {
						switch ($theOptionArray[1]) {
						case 1:
							$opertion = '==';
							break;

						case 2:
							$opertion = '<';
							break;

						case 3:
							$opertion = '<=';
							break;

						case 4:
							$opertion = '>';
							break;

						case 5:
							$opertion = '>=';
							break;

						case 6:
							$opertion = '!=';
							break;
						}

						if ($i == $Con_Total) {
							$opertionRelation = '';
						}
						else {
							$opertionRelation = ' && ';
						}

						$conList .= '( getID(\'option_' . $condOnID . '\') != null && !isNaN(getID(\'option_' . $condOnID . '\').value) && getID(\'option_' . $condOnID . '\').value != \'\' && getID(\'option_' . $condOnID . '\').value ' . $opertion . ' ' . $theOptionArray[0] . ')' . $opertionRelation;
					}
					else {
						if ($i == $Con_Total) {
							$opertionRelation = '';
						}
						else if ($theOptionArray[2] == 1) {
							$opertionRelation = ' && ';
						}
						else {
							$opertionRelation = ' || ';
						}

						switch ($condOnID_QuestionType) {
						case '1':
							if ($theOptionArray[1] == 1) {
								if (issamepage($questionID, $condOnID)) {
									$conList .= '( IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value == ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
							}
							else if (issamepage($questionID, $condOnID)) {
								$conList .= '( !IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value != ' . $theOptionArray[0] . ')' . $opertionRelation;
							}

							break;

						case '2':
							if ($theOptionArray[1] == 1) {
								if (issamepage($questionID, $condOnID)) {
									if ($QtnListArray[$condOnID]['isSelect'] == '1') {
										$conList .= '( IsInSelectedList(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
									}
									else {
										$conList .= '( IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
									}
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value == \'' . $theOptionArray[0] . '\')' . $opertionRelation;
								}
							}
							else if (issamepage($questionID, $condOnID)) {
								if ($QtnListArray[$condOnID]['isSelect'] == '1') {
									$conList .= '( !IsInSelectedList(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( !IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value != ' . $theOptionArray[0] . ')' . $opertionRelation;
							}

							break;

						case '3':
							if ($theOptionArray[3] == 2) {
								switch ($theOptionArray[1]) {
								case 1:
									$opertion = '==';
									break;

								case 2:
									$opertion = '<';
									break;

								case 3:
									$opertion = '<=';
									break;

								case 4:
									$opertion = '>';
									break;

								case 5:
									$opertion = '>=';
									break;

								case 6:
									$opertion = '!=';
									break;
								}

								if ($i == $Con_Total) {
									$opertionRelation = '';
								}
								else {
									$opertionRelation = ' && ';
								}

								if (issamepage($questionID, $condOnID)) {
									$conList .= '(getCheckBoxSelNum(document.Survey_Form.option_' . $condOnID . ') ' . $opertion . ' ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value.split(\',\').length ' . $opertion . ' ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
							}
							else if ($theOptionArray[1] == 1) {
								if (issamepage($questionID, $condOnID)) {
									if ($QtnListArray[$condOnID]['isSelect'] == '1') {
										$conList .= '( IsInSelectedList(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
									}
									else {
										$conList .= '( IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
									}
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && isInValueList(' . $theOptionArray[0] . ',getID(\'option_' . $condOnID . '\').value) )' . $opertionRelation;
								}
							}
							else if (issamepage($questionID, $condOnID)) {
								if ($QtnListArray[$condOnID]['isSelect'] == '1') {
									$conList .= '( !IsInSelectedList(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( !IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '\') != null && !isInValueList(' . $theOptionArray[0] . ',getID(\'option_' . $condOnID . '\').value) )' . $opertionRelation;
							}

							break;

						case '17':
							if ($QtnListArray[$condOnID]['isSelect'] == 1) {
								if ($theOptionArray[1] == 1) {
									if (issamepage($questionID, $condOnID)) {
										$conList .= '( IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
									}
									else {
										$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value == ' . $theOptionArray[0] . ')' . $opertionRelation;
									}
								}
								else if (issamepage($questionID, $condOnID)) {
									$conList .= '( !IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value != ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
							}
							else if ($theOptionArray[1] == 1) {
								if (issamepage($questionID, $condOnID)) {
									$conList .= '( IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && isInValueList(' . $theOptionArray[0] . ',getID(\'option_' . $condOnID . '\').value) )' . $opertionRelation;
								}
							}
							else if (issamepage($questionID, $condOnID)) {
								$conList .= '( !IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '\') != null && !isInValueList(' . $theOptionArray[0] . ',getID(\'option_' . $condOnID . '\').value) )' . $opertionRelation;
							}

							break;

						case '24':
							if ($theOptionArray[1] == 1) {
								if (issamepage($questionID, $condOnID)) {
									$conList .= '( IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value == ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
							}
							else if (issamepage($questionID, $condOnID)) {
								$conList .= '( !IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value != ' . $theOptionArray[0] . ')' . $opertionRelation;
							}

							break;

						case '25':
							if ($theOptionArray[3] == 2) {
								switch ($theOptionArray[1]) {
								case 1:
									$opertion = '==';
									break;

								case 2:
									$opertion = '<';
									break;

								case 3:
									$opertion = '<=';
									break;

								case 4:
									$opertion = '>';
									break;

								case 5:
									$opertion = '>=';
									break;

								case 6:
									$opertion = '!=';
									break;
								}

								if ($i == $Con_Total) {
									$opertionRelation = '';
								}
								else {
									$opertionRelation = ' && ';
								}

								if (issamepage($questionID, $condOnID)) {
									$conList .= '(getCheckBoxSelNum(document.Survey_Form.option_' . $condOnID . ') ' . $opertion . ' ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && getID(\'option_' . $condOnID . '\').value.split(\',\').length ' . $opertion . ' ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
							}
							else if ($theOptionArray[1] == 1) {
								if (issamepage($questionID, $condOnID)) {
									$conList .= '( IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && isInValueList(' . $theOptionArray[0] . ',getID(\'option_' . $condOnID . '\').value) )' . $opertionRelation;
								}
							}
							else if (issamepage($questionID, $condOnID)) {
								$conList .= '( !IsInCheckBox(document.Survey_Form.option_' . $condOnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '\') != null && !isInValueList(' . $theOptionArray[0] . ',getID(\'option_' . $condOnID . '\').value) )' . $opertionRelation;
							}

							break;

						case '30':
							if ($theOptionArray[1] == 1) {
								if ($theOptionArray[0] == 1) {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && parseInt(getID(\'option_' . $condOnID . '\').value) == 1 )' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '\') != null && parseInt(getID(\'option_' . $condOnID . '\').value) == 0 )' . $opertionRelation;
								}
							}
							else if ($theOptionArray[0] == 1) {
								$conList .= '( getID(\'option_' . $condOnID . '\') != null && parseInt(getID(\'option_' . $condOnID . '\').value) != 1 )' . $opertionRelation;
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '\') != null && parseInt(getID(\'option_' . $condOnID . '\').value) != 0 )' . $opertionRelation;
							}

							break;
						}
					}
				}

				if (2 <= $Con_Total) {
					$conList .= ')';
				}

				break;

			case '6':
			case '7':
			case '19':
			case '28':
			case '23':
			case '10':
			case '15':
			case '16':
			case '20':
			case '21':
			case '22':
			case '31':
				$RangeTotal = count($theCondOnIDArray);
				$k = 0;

				if (2 <= $RangeTotal) {
					$conList .= '(';
				}

				foreach ($theCondOnIDArray as $qtnID => $theRangeArray) {
					$i = 0;
					$Con_Total = count($theRangeArray);

					if (2 <= $Con_Total) {
						$conList .= '(';
					}

					foreach ($theRangeArray as $theOptionArray) {
						$i++;

						switch ($condOnID_QuestionType) {
						case '23':
						case '10':
						case '16':
						case '20':
						case '22':
							switch ($theOptionArray[1]) {
							case 1:
								$opertion = '==';
								break;

							case 2:
								$opertion = '<';
								break;

							case 3:
								$opertion = '<=';
								break;

							case 4:
								$opertion = '>';
								break;

							case 5:
								$opertion = '>=';
								break;

							case 6:
								$opertion = '!=';
								break;
							}

							if ($i == $Con_Total) {
								$opertionRelation = '';
							}
							else {
								$opertionRelation = ' && ';
							}

							$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && !isNaN(getID(\'option_' . $condOnID . '_' . $qtnID . '\').value) && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value != \'\' && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value ' . $opertion . ' ' . $theOptionArray[0] . ')' . $opertionRelation;
							break;

						case '15':
						case '21':
							switch ($theOptionArray[1]) {
							case 1:
								$opertion = '==';
								break;

							case 2:
								$opertion = '<';
								break;

							case 3:
								$opertion = '<=';
								break;

							case 4:
								$opertion = '>';
								break;

							case 5:
								$opertion = '>=';
								break;

							case 6:
								$opertion = '!=';
								break;
							}

							if ($i == $Con_Total) {
								$opertionRelation = '';
							}
							else {
								$opertionRelation = ' && ';
							}

							switch ($QtnListArray[$condOnID]['isSelect']) {
							case '0':
								if (issamepage($questionID, $condOnID)) {
									$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && getRadioCheckBoxValue(document.Survey_Form.option_' . $condOnID . '_' . $qtnID . ') != \'\' && getRadioCheckBoxValue(document.Survey_Form.option_' . $condOnID . '_' . $qtnID . ') != 99 && parseInt(getRadioCheckBoxValue(document.Survey_Form.option_' . $condOnID . '_' . $qtnID . ') * ' . $QtnListArray[$condOnID]['weight'] . ') ' . $opertion . ' ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value != \'\' && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value != 99 && parseInt(getID(\'option_' . $condOnID . '_' . $qtnID . '\').value * ' . $QtnListArray[$condOnID]['weight'] . ') ' . $opertion . ' ' . $theOptionArray[0] . ')' . $opertionRelation;
								}

								break;

							case '1':
							case '2':
								$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && !isNaN(getID(\'option_' . $condOnID . '_' . $qtnID . '\').value) && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value != \'\' && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value ' . $opertion . ' ' . $theOptionArray[0] . ')' . $opertionRelation;
								break;
							}

							break;

						case '6':
						case '19':
							if ($i == $Con_Total) {
								$opertionRelation = '';
							}
							else if ($theOptionArray[2] == 1) {
								$opertionRelation = ' && ';
							}
							else {
								$opertionRelation = ' || ';
							}

							if ($theOptionArray[1] == 1) {
								if (issamepage($questionID, $condOnID)) {
									$conList .= '( IsInCheckBox(document.Survey_Form.option_' . $condOnID . '_' . $qtnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value == ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
							}
							else if (issamepage($questionID, $condOnID)) {
								$conList .= '( !IsInCheckBox(document.Survey_Form.option_' . $condOnID . '_' . $qtnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value != ' . $theOptionArray[0] . ')' . $opertionRelation;
							}

							break;

						case '7':
						case '28':
							if ($i == $Con_Total) {
								$opertionRelation = '';
							}
							else if ($theOptionArray[2] == 1) {
								$opertionRelation = ' && ';
							}
							else {
								$opertionRelation = ' || ';
							}

							if ($theOptionArray[1] == 1) {
								if (issamepage($questionID, $condOnID)) {
									$conList .= '( IsInCheckBox(document.Survey_Form.option_' . $condOnID . '_' . $qtnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && isInValueList(' . $theOptionArray[0] . ',getID(\'option_' . $condOnID . '_' . $qtnID . '\').value) )' . $opertionRelation;
								}
							}
							else if (issamepage($questionID, $condOnID)) {
								$conList .= '( !IsInCheckBox(document.Survey_Form.option_' . $condOnID . '_' . $qtnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && !isInValueList(' . $theOptionArray[0] . ',getID(\'option_' . $condOnID . '_' . $qtnID . '\').value) )' . $opertionRelation;
							}

							break;

						case '31':
							if ($i == $Con_Total) {
								$opertionRelation = '';
							}
							else if ($theOptionArray[2] == 1) {
								$opertionRelation = ' && ';
							}
							else {
								$opertionRelation = ' || ';
							}

							if ($theOptionArray[1] == 1) {
								if (issamepage($questionID, $condOnID)) {
									$conList .= '( IsInSelectedList(document.Survey_Form.option_' . $condOnID . '_' . $qtnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
								}
								else {
									$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value == ' . $theOptionArray[0] . ')' . $opertionRelation;
								}
							}
							else if (issamepage($questionID, $condOnID)) {
								$conList .= '( !IsInSelectedList(document.Survey_Form.option_' . $condOnID . '_' . $qtnID . ',' . $theOptionArray[0] . ') )' . $opertionRelation;
							}
							else {
								$conList .= '( getID(\'option_' . $condOnID . '_' . $qtnID . '\') != null && getID(\'option_' . $condOnID . '_' . $qtnID . '\').value != ' . $theOptionArray[0] . ')' . $opertionRelation;
							}

							break;
						}
					}

					if (2 <= $Con_Total) {
						$conList .= ')';
					}

					$k++;

					if ($k != $RangeTotal) {
						$conList .= ' && ';
					}
				}

				if (2 <= $RangeTotal) {
					$conList .= ')';
				}

				break;
			}

			$j++;

			if ($j != count($QuotaListArray[$theQuotaID])) {
				$conList .= ' && ';
			}
		}

		if ($conList != '') {
			$endConList .= '		if(' . $conList . ')' . "\n" . '		{' . "\n" . '';
			$endConList .= '			thisCheckStr += \'0*' . $theQuotaID . '$' . $questionID . '$' . $theMgtFunc . '$' . $theResponseID . '|\';' . "\n" . '';
			$endConList .= '		}' . "\n" . '';
		}
		else {
			$endConList .= '';
		}
	}

	return $endConList;
}

if (!defined('ROOT_PATH')) {
	exit('EnableQ Security Violation');
}

?>
