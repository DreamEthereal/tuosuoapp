...
$EnableQCoreClass->setTemplateFile('StatDayFile', 'StatsDayReport.html');
...
if ($_POST['qyear'] && $_POST['qmonth'] && $_POST['qday']) {
	$EnableQCoreClass->replace('today', $_POST['qyear'] . '-' . $_POST['qmonth'] . '-' . $_POST['qday']);
}
else {
	$EnableQCoreClass->replace('today', date('Y-n-j'));
}

$EnableQCoreClass->pparse('StatDay', 'StatDayFile');
